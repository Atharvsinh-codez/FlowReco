...
## D021 — Keep unsigned CI installers separate from signed releases

**Status:** Accepted, 2026-07-14
**Decision:** A dedicated Windows CI workflow may build and retain an unsigned x64 NSIS installer for hands-on testing. Public releases continue to require the existing signing and release process.
**Why:** Contributors need a reproducible Windows artifact without access to project signing secrets, while users must not mistake a development build for a trusted signed release.
**Consequence:** CI verifies that exactly one non-trivial installer exists, records its SHA-256 checksum, labels it unsigned, and never publishes it as a stable release.
