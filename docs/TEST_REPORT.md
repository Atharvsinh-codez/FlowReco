...
### Windows unsigned installer CI

`.github/workflows/windows-installer.yml` provisions a Windows 2022 runner, Rust 1.88.0, the Windows native dependencies, the FlowReco sidecars, and an unsigned x64 NSIS bundle. The job rejects a missing, duplicate, or implausibly small installer and uploads the installer with `SHA256SUMS.txt` as `FlowReco-Windows-x64`.

The workflow configuration and its dedicated Tauri override were checked locally for JSON validity, repository paths, target consistency, and whitespace errors. Execution remains pending until the source is available to the Windows runner; a successful workflow run and downloaded-artifact hash verification are required before the executable is handed off.
