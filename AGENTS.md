# FlowReco Repository Guidelines

FlowReco is a Cap-derived codebase in a clean Git history. User-facing product identity is
FlowReco, while internal compatibility identifiers such as `@cap/*`, Rust crate names such
as `cap-*`, the `cap-setup` script, generated bindings, persisted schemas, and some
`CAP_*` environment variables intentionally retain their upstream names for now. Do not
rename them mechanically. Change an internal identifier only with an explicit migration
plan, compatibility coverage, and an entry in `docs/DECISIONS.md`.
...
- FlowReco Turborepo monorepo, derived from the pinned Cap foundation:
...
## Build, Test, Develop
- Install: `pnpm install`; setup: `pnpm env-setup` then `pnpm cap-setup` (the
  compatibility script name is intentional).
...
## Commits & PRs
- Conventional style: `feat:`, `fix:`, `chore:`, `improve:`, `refactor:`, `docs:` (e.g., `fix: preserve audio sync across pause`).
