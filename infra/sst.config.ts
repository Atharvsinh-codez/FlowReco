...
if (process.env.FLOWRECO_ENABLE_MANAGED_INFRA !== "1") {
	throw new Error(
		"Managed SST deployment is disabled until FlowReco-owned cloud accounts are configured. Use Docker Compose for self-hosting.",
	);
}

const GITHUB_ORG = "Atharvsinh-codez";
const _GITHUB_REPO = "FlowReco";
const _GITHUB_APP_ID = process.env.FLOWRECO_GITHUB_APP_ID ?? "";

const VERCEL_PROJECT_NAME = "flowreco-web";
const VERCEL_TEAM_SLUG = process.env.FLOWRECO_VERCEL_TEAM_SLUG ?? "";
const VERCEL_TEAM_ID = process.env.FLOWRECO_VERCEL_TEAM_ID ?? "";

const _CLOUDFLARE_ACCOUNT_ID = process.env.FLOWRECO_CLOUDFLARE_ACCOUNT_ID ?? "";
const AXIOM_API_TOKEN = process.env.FLOWRECO_AXIOM_API_TOKEN ?? "";
const AXIOM_DATASET = "flowreco-otel";
