...
// FlowReco does not redistribute model weights without exact source and license
// provenance. Background blur degrades to the existing unblurred fallback until a
// reviewed model is supplied through a future asset installer.
...
fn create_session() -> anyhow::Result<Session> {
    anyhow::ensure!(
        ...
        "Camera background segmentation is unavailable: no license-cleared model is bundled"
    );
    init_runtime()?;
