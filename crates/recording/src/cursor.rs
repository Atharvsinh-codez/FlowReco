...
pub fn spawn_cursor_recorder(
    ...
    let thread = std::thread::spawn(move || {
        ...
        let mut last_keys: Vec<device_query::Keycode> =
            poll_keyboard_if_enabled(keyboard_capture, || device_state.get_keys());
...
fn poll_keyboard_if_enabled<T>(enabled: bool, poll: impl FnOnce() -> Vec<T>) -> Vec<T> {
    if enabled { poll() } else { Vec::new() }
}

#[cfg(test)]
mod tests {
    use super::poll_keyboard_if_enabled;
    use std::sync::atomic::{AtomicBool, Ordering};

    #[test]
    fn disabled_keyboard_capture_never_polls_device_state() {
        let polled = AtomicBool::new(false);
        let keys: Vec<u8> = poll_keyboard_if_enabled(false, || {
    ...
    #[test]
    fn enabled_keyboard_capture_polls_device_state() {
        let polled = AtomicBool::new(false);
        let keys = poll_keyboard_if_enabled(true, || {
            polled.store(true, Ordering::SeqCst);
            vec![1_u8]
        });
