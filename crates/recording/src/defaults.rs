...
/// Exact key capture is sensitive and must always require an explicit opt-in.
pub const DEFAULT_CAPTURE_KEYBOARD_EVENTS: bool = false;
...
mod tests {
    ...
    #[test]
    fn keyboard_capture_is_opt_in_by_default() {
        assert!(!DEFAULT_CAPTURE_KEYBOARD_EVENTS);
        assert!(!RecordingDefaults::default().capture_keyboard_events);
    }
}
