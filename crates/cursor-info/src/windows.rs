...
const POINTER_HOTSPOT: (f64, f64) = (0.125, 0.09375);
const CENTER_HOTSPOT: (f64, f64) = (0.5, 0.5);
const POINTING_HAND_HOTSPOT: (f64, f64) = (0.4375, 0.125);
const UP_ARROW_HOTSPOT: (f64, f64) = (0.5, 0.15625);
const PIN_HOTSPOT: (f64, f64) = (0.5, 0.875);
const PEN_HOTSPOT: (f64, f64) = (0.21875, 0.8125);
...
mod tests {
    ...
    const SUPPORTED: [CursorShapeWindows; 17] = [
        CursorShapeWindows::Arrow,
        CursorShapeWindows::IBeam,
        CursorShapeWindows::Wait,
        CursorShapeWindows::Cross,
        CursorShapeWindows::UpArrow,
        CursorShapeWindows::SizeNWSE,
        CursorShapeWindows::SizeNESW,
        ...
        CursorShapeWindows::AppStarting,
        CursorShapeWindows::Help,
        CursorShapeWindows::Pin,
        CursorShapeWindows::Person,
        CursorShapeWindows::Pen,
    ];

    const UNSUPPORTED: [CursorShapeWindows; 12] = [
        CursorShapeWindows::ScrollNS,
        CursorShapeWindows::ScrollWE,
        CursorShapeWindows::ScrollNSEW,
        CursorShapeWindows::ScrollN,
    ...
    #[test]
    fn supported_assets_are_original_reco_svgs_with_bounded_hotspots() {
        for shape in SUPPORTED {
            let resolved = shape.resolve().expect("supported cursor must resolve");

            assert!(resolved.raw.contains("Original FlowReco cursor artwork"));
            assert!(resolved.raw.contains("viewBox=\"0 0 32 32\""));
            assert!(!resolved.raw.contains("<image"));
            assert!((0.0..=1.0).contains(&resolved.hotspot.0));
            assert!((0.0..=1.0).contains(&resolved.hotspot.1));
        }
    }

    #[test]
    fn hotspots_match_reco_asset_geometry() {
        assert_hotspot(CursorShapeWindows::Arrow, (4.0 / 32.0, 3.0 / 32.0));
        assert_hotspot(
            CursorShapeWindows::Hand,
            (14.0 / 32.0, 4.0 / 32.0),
        );
        ...
        assert_hotspot(CursorShapeWindows::Pen, (7.0 / 32.0, 26.0 / 32.0));
    }

    #[test]
    fn unimplemented_platform_shapes_remain_unresolved() {
        for shape in UNSUPPORTED {
            assert!(shape.resolve().is_none());
        }
    }

    fn assert_hotspot(shape: CursorShapeWindows, expected: (f64, f64)) {
        let actual = shape
            .resolve()
            .expect("supported cursor must resolve")
            .hotspot;
