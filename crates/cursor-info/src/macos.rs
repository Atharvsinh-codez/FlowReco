...
const OPEN_HAND_HOTSPOT: (f64, f64) = (0.5, 0.4375);
const MAGNIFIER_HOTSPOT: (f64, f64) = (0.4375, 0.4375);
...
mod tests {
    ...
    const SUPPORTED: [CursorShapeMacOS; 36] = [
    ...
    fn hotspots_match_reco_asset_geometry() {
        assert_hotspot(CursorShapeMacOS::Arrow, (4.0 / 32.0, 3.0 / 32.0));
        assert_hotspot(
            CursorShapeMacOS::PointingHand,
            (14.0 / 32.0, 4.0 / 32.0),
        );
    ...
    #[test]
    fn disappearing_item_variants_remain_intentionally_unresolved() {
        assert!(CursorShapeMacOS::DisappearingItem.resolve().is_none());
        assert!(CursorShapeMacOS::TahoeDisappearingItem.resolve().is_none());
    }

    fn assert_hotspot(shape: CursorShapeMacOS, expected: (f64, f64)) {
        let actual = shape
            .resolve()
