...
fn apply_env_headers(req: reqwest::RequestBuilder) -> reqwest::RequestBuilder {
    let mut req = req
        .header("X-FlowReco-Desktop-Version", env!("CARGO_PKG_VERSION"))
        .header("X-FlowReco-Desktop-Features", "googleDriveUpload")
        // Preserve upstream headers while self-hosted servers transition.
