...
const UPDATE_ENDPOINT: Option<&str> = option_env!("RECO_UPDATE_ENDPOINT");
...
// target reports "macos"/"linux" while FlowReco release feeds are keyed on
// "darwin-*" / "linux-*-deb".
...
fn endpoint_from_base(base: Option<&str>, channel: UpdateChannel) -> Result<Url, String> {
    let base = base.filter(|value| !value.trim().is_empty()).ok_or_else(|| {
        "Automatic updates are not configured for this build. Install releases from a trusted FlowReco distribution channel."
            .to_string()
    })?;
    let url = match channel {
        UpdateChannel::Stable => base.to_string(),
        UpdateChannel::Nightly => format!("{base}?channel=nightly"),
    };
    Url::parse(&url).map_err(|e| e.to_string())
}

fn endpoint(channel: UpdateChannel) -> Result<Url, String> {
    endpoint_from_base(UPDATE_ENDPOINT, channel)
}
        ...
        loop {
            ...
            let installed = if cfg!(target_os = "macos") {
                ...
                if already_installed {
                    ...
                    // Safe while FlowReco runs: the .app bundle is swapped in place
...
mod tests {
    ...
    #[test]
    fn update_endpoint_requires_an_explicit_reco_feed() {
        let error = endpoint_from_base(None, UpdateChannel::Stable).unwrap_err();
        assert!(error.contains("not configured"));
    }

    #[test]
    fn update_endpoint_adds_the_nightly_channel() {
        let url = endpoint_from_base(
            Some("https://updates.example/reco/{{target}}/{{current_version}}"),
            UpdateChannel::Nightly,
        )
        .unwrap();

        assert_eq!(url.query(), Some("channel=nightly"));
        assert_eq!(url.host_str(), Some("updates.example"));
    }
}
