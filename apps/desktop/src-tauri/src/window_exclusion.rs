...
mod tests {
    ...
    fn matches_by_window_title() {
        let exclusion = title_exclusion("FlowReco Camera");
        assert!(exclusion.matches(None, None, Some("FlowReco Camera")));
    ...
    fn matches_by_owner_name() {
        let exclusion = owner_exclusion("FlowReco");
        assert!(exclusion.matches(None, Some("FlowReco"), None));
    ...
    fn matches_owner_and_title_requires_both() {
        let exclusion = WindowExclusion {
            bundle_identifier: None,
            owner_name: Some("FlowReco".to_string()),
            window_title: Some("FlowReco Camera".to_string()),
        };
        assert!(exclusion.matches(None, Some("FlowReco"), Some("FlowReco Camera")));
        assert!(!exclusion.matches(None, Some("FlowReco"), Some("Wrong Title")));
        assert!(!exclusion.matches(None, Some("Wrong Owner"), Some("FlowReco Camera")));
        assert!(!exclusion.matches(None, None, Some("FlowReco Camera")));
    }
    ...
    fn instant_mode_removes_camera_exclusion() {
        let exclusions = vec![
            title_exclusion("FlowReco"),
            title_exclusion("FlowReco Camera"),
            title_exclusion("FlowReco Settings"),
            title_exclusion("FlowReco Recording Controls"),
        ];

        let filtered = filter_for_instant_mode(exclusions, "FlowReco Camera");

        assert_eq!(filtered.len(), 3);
        assert!(
            filtered
                .iter()
                .all(|e| e.window_title.as_deref() != Some("FlowReco Camera"))
        );
        assert!(
            filtered
                .iter()
                .any(|e| e.window_title.as_deref() == Some("FlowReco"))
        );
        assert!(
            filtered
                .iter()
                .any(|e| e.window_title.as_deref() == Some("FlowReco Settings"))
        );
        assert!(
            filtered
                .iter()
                .any(|e| e.window_title.as_deref() == Some("FlowReco Recording Controls"))
        );
    }

    #[test]
    fn instant_mode_noop_when_camera_absent() {
        let exclusions = vec![title_exclusion("FlowReco"), title_exclusion("FlowReco Settings")];
    ...
    fn instant_mode_handles_empty_list() {
        let filtered = filter_for_instant_mode(vec![], "FlowReco Camera");
        assert!(filtered.is_empty());
    }

    #[test]
    fn matches_webview_title_using_exclusion_rules() {
        let exclusions = vec![title_exclusion("FlowReco Camera")];

        assert!(matches_window_title(&exclusions, "FlowReco Camera"));
        assert!(!matches_window_title(&exclusions, "FlowReco Recording Controls"));
    }
    ...
    fn default_exclusions_contain_camera() {
        ...
        assert!(
            defaults
                .iter()
                .any(|e| e.window_title.as_deref() == Some("FlowReco Camera")),
            "Default exclusions must include 'FlowReco Camera' — instant mode filtering depends on this"
        );
    }
    ...
    fn default_exclusions_only_match_camera_by_title() {
        ...
        let camera = defaults
            .iter()
            .find(|e| e.window_title.as_deref() == Some("FlowReco Camera"))
            .expect("FlowReco Camera must be in default exclusions");
