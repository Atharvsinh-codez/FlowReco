...
impl CapWindowId {
    ...
    pub fn title(&self) -> String {
        match self {
			Self::Settings => "FlowReco Settings".to_string(),
			Self::WindowCaptureOccluder { .. } => "FlowReco Window Capture Occluder".to_string(),
			Self::CaptureArea => "FlowReco Capture Area".to_string(),
			Self::RecordingControls => "FlowReco Recording Controls".to_string(),
			Self::Editor { .. } => "FlowReco Editor".to_string(),
			Self::ScreenshotEditor { .. } => "FlowReco Screenshot Editor".to_string(),
			Self::ModeSelect => "FlowReco Mode Selection".to_string(),
			Self::Onboarding => "Welcome to FlowReco".to_string(),
			Self::Camera => "FlowReco Camera".to_string(),
			Self::RecordingsOverlay => "FlowReco Recordings Overlay".to_string(),
			Self::TargetSelectOverlay { .. } => "FlowReco Target Select".to_string(),
			_ => "FlowReco".to_string(),
        }
    }
...
// FlowReco's own windows while a recording is actually active, which is the only time the
...
// is skipped entirely there — FlowReco's windows then appear in recordings, which is the
        ...
        match &reason {
            Some(reason) => warn!(
                ...
                "Skipping window capture exclusion: this desktop is viewed through a \
                 ...
                 FlowReco's windows will appear in recordings."
            ),
