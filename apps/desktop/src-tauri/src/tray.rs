        ...
        return Menu::with_items(
            ...
            &[
                ...
                &MenuItem::with_id(
                    app,
                    "version",
                    format!("FlowReco v{}", env!("CARGO_PKG_VERSION")),
                    false,
                    None::<&str>,
                )?,
                &MenuItem::with_id(app, TrayItem::Quit, "Quit FlowReco", true, None::<&str>)?,
            ],
        );
    }
...
fn set_linux_tray_icon(tray: &TrayIcon<tauri::Wry>, icon: LinuxTrayIcon) -> tauri::Result<()> {
    ...
    tray.with_inner_tray_icon(move |inner| unsafe {
        ...
        if let Some(indicator) = indicator.as_mut() {
            ...
            indicator.set_icon_full(&icon_name, "FlowReco tray icon");
        }
    })?;
