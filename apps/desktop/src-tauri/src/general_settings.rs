...
pub(crate) const KEYBOARD_CAPTURE_OPT_IN_MIGRATION_KEY: &str =
    "reco_keyboard_capture_explicit_opt_in_v1";
pub(crate) const TELEMETRY_OPT_IN_MIGRATION_KEY: &str =
    "reco_telemetry_explicit_opt_in_v1";
const SERVER_URL_MIGRATION_KEY: &str = "flowreco_server_url_local_first_v1";
const DEFAULT_LOCAL_SERVER_URL: &str = "http://localhost:3000";
...
// NOTE: Do not add "FlowReco Target Select" here — on Windows, WDA_EXCLUDEFROMCAPTURE applied to that
// hidden window causes it to reappear as a ghost overlay after recording ends.
const DEFAULT_EXCLUDED_WINDOW_TITLES: &[&str] = &[
    "FlowReco",
    "FlowReco Settings",
    "FlowReco Recording Controls",
    "FlowReco Camera",
    "FlowReco Window Capture Occluder",
    "FlowReco Capture Area",
    "FlowReco Mode Selection",
    "FlowReco Recordings Overlay",
];

const LEGACY_CAP_WINDOW_TITLES: &[&str] = &[
    "Cap",
    "Cap Settings",
...
fn default_server_url() -> String {
    std::option_env!("VITE_SERVER_URL")
        .unwrap_or(DEFAULT_LOCAL_SERVER_URL)
        .to_string()
}
...
pub fn init(app: &AppHandle) {
    ...
    let (keyboard_migrated, telemetry_migrated, server_url_migrated) = app
        .store("store")
        .ok()
        .map(|raw_store| {
            (
                raw_store
                    .get(KEYBOARD_CAPTURE_OPT_IN_MIGRATION_KEY)
                    .and_then(|value| value.as_bool())
                    == Some(true),
                raw_store
                    .get(TELEMETRY_OPT_IN_MIGRATION_KEY)
                    .and_then(|value| value.as_bool())
                    == Some(true),
                raw_store
                    .get(SERVER_URL_MIGRATION_KEY)
                    .and_then(|value| value.as_bool())
                    == Some(true),
            )
        })
        .unwrap_or((false, false, false));
    let (keyboard_reset, telemetry_reset) = reset_inherited_privacy_settings(
        &mut store,
        keyboard_migrated,
        telemetry_migrated,
    );
    let server_url_reset = reset_inherited_server_url(&mut store, server_url_migrated);

    if let Ok(raw_store) = app.store("store") {
        if keyboard_reset {
            raw_store.set(KEYBOARD_CAPTURE_OPT_IN_MIGRATION_KEY, json!(true));
        }
        if telemetry_reset {
            raw_store.set(TELEMETRY_OPT_IN_MIGRATION_KEY, json!(true));
        }
        if server_url_reset {
            raw_store.set(SERVER_URL_MIGRATION_KEY, json!(true));
        }

        const WINDOW_EXCLUSIONS_MIGRATION_KEY: &str =
            "flowreco_window_exclusions_v1";
        if raw_store.get(WINDOW_EXCLUSIONS_MIGRATION_KEY).is_none() {
            store.excluded_windows.retain(|window| {
                window
                    .window_title
                    .as_deref()
                    .is_none_or(|title| !LEGACY_CAP_WINDOW_TITLES.contains(&title))
            });
            raw_store.set(WINDOW_EXCLUSIONS_MIGRATION_KEY, json!(true));
        }
    }
...
fn reset_inherited_privacy_settings(
    store: &mut GeneralSettingsStore,
    keyboard_migrated: bool,
    telemetry_migrated: bool,
) -> (bool, bool) {
    if !keyboard_migrated {
        store.capture_keyboard_events = false;
    }
    if !telemetry_migrated {
        store.enable_telemetry = false;
    }

    (!keyboard_migrated, !telemetry_migrated)
}

fn reset_inherited_server_url(store: &mut GeneralSettingsStore, migrated: bool) -> bool {
    if migrated {
        return false;
    }

    if store.server_url.trim_end_matches('/') == "https://cap.so" {
        store.server_url = default_server_url();
    }
...
mod tests {
    ...
    #[test]
    fn privacy_sensitive_settings_default_to_disabled() {
        let defaults = GeneralSettingsStore::default();
        assert!(!defaults.capture_keyboard_events);
        assert!(!defaults.enable_telemetry);

        let deserialized: GeneralSettingsStore = serde_json::from_value(json!({})).unwrap();
        assert!(!deserialized.capture_keyboard_events);
        assert!(!deserialized.enable_telemetry);
    }

    #[test]
    fn first_privacy_migration_clears_inherited_opt_ins() {
        let mut settings = GeneralSettingsStore {
        ...
        assert_eq!(
            reset_inherited_privacy_settings(&mut settings, false, false),
            (true, true)
        );
        assert!(!settings.capture_keyboard_events);
        assert!(!settings.enable_telemetry);
    }

    #[test]
    fn completed_privacy_migrations_preserve_explicit_choices() {
        let mut settings = GeneralSettingsStore {
            capture_keyboard_events: true,
            enable_telemetry: true,
            ..Default::default()
        ...
        assert_eq!(
            reset_inherited_privacy_settings(&mut settings, true, true),
            (false, false)
        );
        assert!(settings.capture_keyboard_events);
        assert!(settings.enable_telemetry);
    ...
    #[test]
    fn first_server_migration_replaces_inherited_cap_cloud() {
        let mut settings = GeneralSettingsStore {
            server_url: "https://cap.so/".to_string(),
            ..Default::default()
        };

        assert!(reset_inherited_server_url(&mut settings, false));
        assert_eq!(settings.server_url, default_server_url());
    }

    #[test]
    fn server_migration_preserves_explicit_and_custom_origins() {
        let mut completed = GeneralSettingsStore {
            server_url: "https://cap.so".to_string(),
            ..Default::default()
        };
        assert!(!reset_inherited_server_url(&mut completed, true));
        assert_eq!(completed.server_url, "https://cap.so");

        let mut custom = GeneralSettingsStore {
            server_url: "https://recordings.example.com".to_string(),
            ..Default::default()
        };
        assert!(reset_inherited_server_url(&mut custom, false));
        assert_eq!(custom.server_url, "https://recordings.example.com");
    }
}
