...
mod tests {
    ...
    #[test]
    fn parses_reco_and_legacy_cap_recording_dates() {
        let expected = NaiveDate::from_ymd_opt(2026, 7, 14).unwrap();
        assert_eq!(
            parse_recording_date("FlowReco 2026-07-14 at 09.30.12"),
            Some(expected)
        );
        assert_eq!(
            parse_recording_date("Cap 2026-07-14 at 09.30.12"),
            Some(expected)
        );
    }

    #[test]
    fn rejects_unknown_or_malformed_recording_names() {
        assert_eq!(parse_recording_date("Recording 2026-07-14"), None);
        assert_eq!(parse_recording_date("FlowReco not-a-date at 09.30.12"), None);
    }

    #[test]
    fn applies_the_recovery_cutoff_to_reco_recordings() {
        assert!(is_recording_after_cutoff("FlowReco 2026-01-01 at 00.00.01"));
        assert!(!is_recording_after_cutoff(
            "FlowReco 2025-12-31 at 23.59.59"
        ));
    }
}
