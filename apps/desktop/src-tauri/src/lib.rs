...
fn build_macos_app_menu(app_handle: &AppHandle) -> tauri::Result<Menu<tauri::Wry>> {
    ...
    let quit_label = config
        ...
        .unwrap_or_else(|| "Quit FlowReco".to_string());
...
fn configure_windows_graphics_recovery(
    ...
    if should_engage_graphics_recovery(previous_termination) {
        ...
        warn!(
            "Previous FlowReco session terminated during GPU initialisation; using Windows software graphics recovery mode for this launch"
        );
...
fn configure_camera_blur_recovery(
    ...
    if next.is_some() {
        ...
        if stored.is_none() {
            error!(
                "Previous FlowReco session died with camera background blur active; disabling blur until the next app update"
            );
        } else {
    ...
    #[cfg(not(target_os = "linux"))]
    {
        builder = builder.plugin(tauri_plugin_single_instance::init(|app, args, _cwd| {
            ...
            let action_urls = args
                .iter()
				.filter(|arg| {
					arg.starts_with("flowreco-desktop://")
						|| arg.starts_with("cap-desktop://")
				})
