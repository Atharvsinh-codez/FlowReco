...
import {
	...
	clampZoomAmount,
	copyZoomSettings,
	EDGE_SAFE_MARGIN_DEFAULT,
	EDGE_SAFE_MARGIN_MAX,
	formatZoomAmount,
	parseZoomAmount,
	readCopiedZoomSettings,
	resetZoomSettings,
	ZOOM_AMOUNT_MAX,
	ZOOM_AMOUNT_MIN,
	ZOOM_AMOUNT_PRESETS,
...
const WALLPAPER_NAMES = [
	"flowreco/ember-grid",
	"flowreco/graphite-orbit",
	"flowreco/mineral-night",
	"flowreco/signal-wave",
	"flowreco/coral-field",
	"flowreco/icefield",
] as const;
...
export function ConfigSidebar() {
	...
	const inspectorTabs = () =>
		[
			{ id: TAB_IDS.background, icon: IconCapImage, label: "Scene" },
			{
				id: TAB_IDS.camera,
				icon: IconCapCamera,
				label: "Camera",
			...
			{ id: TAB_IDS.audio, icon: IconCapAudioOn, label: "Audio" },
			{
				id: TAB_IDS.keyboard,
	...
	return (
		...
		>
					...
					<For each={inspectorTabs()}>
						{(item) => (
							<KTabs.Trigger
								value={item.id}
								class={cx(
									"group relative flex size-9 items-center justify-center rounded-[10px] transition-all duration-150 disabled:opacity-40 focus:outline-hidden",
									"text-[var(--recorder-muted,#879192)] data-selected:text-[var(--sleek-accent,#0084d1)]",
									"hover:text-[var(--recorder-text,#252b31)] hover:bg-black/[0.04] dark:hover:bg-white/5",
									"data-selected:bg-[var(--sleek-accent-soft,rgba(0,132,209,0.12))]",
								)}
								onClick={() => {
							...
							>
								<span
									class={cx(
										"absolute left-0 top-1/2 h-4 w-0.5 -translate-y-1/2 rounded-full bg-[var(--sleek-accent,#0084d1)] transition-opacity",
			...
			<div class="flex flex-col flex-1 min-w-0 min-h-0">
				<div class="flex items-center justify-between px-3.5 h-10 shrink-0 border-b border-[var(--recorder-border,#e6e6e6)] dark:border-gray-3">
					<p class="text-[11px] font-semibold uppercase tracking-[0.1em] text-[var(--recorder-muted,#879192)]">
						{inspectorTabs().find((t) => t.id === state.selectedTab)?.label ??
							"Inspector"}
					</p>
				</div>
				<div
                ...
                <Subfield name="Improve Mic Quality">
                  <Toggle disabled />
					...
					<KTabs.Content
						value="cursor"
						class="flex flex-col flex-1 gap-5 p-3.5 min-h-0"
					>
						<div class="flex items-center justify-between">
							<span class="text-[11px] font-semibold uppercase tracking-[0.1em] text-[var(--recorder-muted,#879192)]">
								Cursor
							</span>
							<Toggle
						...
						<Show when={!project.cursor.hide}>
							<Field name="Style" icon={<IconCapCursor />}>
								<div class="grid grid-cols-2 gap-2">
									<For each={CURSOR_TYPE_OPTIONS}>
										{(option) => (
											<button
												type="button"
												class={cx(
													"rounded-[12px] border px-3 py-2.5 text-left transition-all",
													project.cursor.type === option.value
														? "border-[var(--sleek-accent,#0084d1)] bg-[var(--sleek-accent-soft,rgba(0,132,209,0.12))]"
														: "border-[var(--recorder-border,#e6e6e6)] bg-[var(--flow-surface-light,#f5f5f5)] hover:border-gray-5",
												)}
												onClick={() =>
													setProject(
														"cursor",
														"type",
														option.value as CursorType,
													)
												}
											>
												<span class="block text-[13px] font-medium text-gray-12">
													{option.label}
												</span>
												<span class="block mt-0.5 text-[11px] leading-snug text-gray-11">
													{option.description}
												</span>
											</button>
										)}
							...
							>
								<Slider
									value={[project.cursor.motionBlur ?? DEFAULT_MOTION_BLUR]}
									onChange={(v) => {
										const value = v[0];
							...
							<div class="grid grid-cols-2 gap-2">
								<For
									each={CURSOR_ANIMATION_STYLE_OPTIONS.filter(
										(o) => o.value !== "custom",
									)}
								>
									{(option) => (
										<button
											...
											onClick={() =>
												applyCursorStylePreset(
													option.value as CursorAnimationStyle,
												)
											}
										>
											<span class="block text-[13px] font-semibold text-gray-12">
												{option.label}
											</span>
											<span class="block mt-0.5 text-[11px] leading-snug text-gray-11">
												{option.description}
							...
							<KCollapsible open={!project.cursor.raw}>
								...
								<KCollapsible.Content class="overflow-hidden opacity-0 transition-opacity animate-collapsible-up data-expanded:animate-collapsible-down data-expanded:opacity-100">
									<div class="flex flex-col gap-4 pt-3 pb-2 rounded-[12px] border border-[var(--recorder-border,#e6e6e6)] bg-[var(--flow-surface-light,#f5f5f5)] px-3">
										<p class="text-[11px] text-[var(--recorder-muted,#879192)]">
											Spring physics — higher tension is snappier; higher mass
											is heavier; friction damps overshoot.
										</p>
										<Field name="Spring Tension">
											<Slider
                  ...
                  >
					...
					<KTabs.Content value="hotkeys" class="flex flex-1 p-4 min-h-0">
						<Field name="Hotkeys" icon={<IconCapHotkeys />}>
							<p class="text-[12px] leading-relaxed text-[var(--recorder-muted,#879192)]">
								System-wide shortcuts are free to configure in Settings →
								Shortcuts. All capture and edit tools are unlocked with no plan
								gate.
							</p>
						</Field>
					</KTabs.Content>
...
function BackgroundConfig(props: {
	...
	const [backgroundTab, setBackgroundTab] =
		createSignal<keyof typeof BACKGROUND_THEMES>("flowreco");
...
function MultiZoomSegmentConfig(props: {
	segments: Array<{ index: number; segment: ZoomSegment }>;
}) {
	const { setProject } = useEditorContext();
	const [batchFocus, setBatchFocus] = createSignal<XY<number>>({
		x: 0.5,
		y: 0.5,
	});

	const updateSelected = (update: (segment: ZoomSegment) => ZoomSegment) => {
		const selected = new Set(props.segments.map(({ index }) => index));
		setProject(
			"timeline",
			"zoomSegments",
			produce((segments) => {
				for (let index = 0; index < segments.length; index++) {
					...
					if (segment && selected.has(index)) segments[index] = update(segment);
				}
			}),
		);
	};
	...
	const pasteSettings = () => {
		const copied = readCopiedZoomSettings();
		if (!copied) {
			toast("Copy a zoom style first");
			return;
		}
		updateSelected((segment) => applyZoomSettings(segment, copied));
		toast.success(`Style applied to ${props.segments.length} zooms`);
	};

	return (
		<div class="space-y-4 rounded-xl border border-gray-4 bg-gray-2/70 p-4">
			<div>
				<p class="text-sm font-medium text-gray-12">Batch zoom controls</p>
				<p class="mt-0.5 text-xs text-gray-10">
					Changes apply to all {props.segments.length} selected regions.
				</p>
			</div>

			<div class="grid grid-cols-6 gap-1.5">
				<For each={ZOOM_AMOUNT_PRESETS}>
					{(amount) => (
						<button
							type="button"
							class="rounded-lg border border-gray-3 bg-gray-1 px-1 py-1.5 text-[11px] text-gray-11 transition-colors hover:border-gray-6 hover:text-gray-12"
							onClick={() =>
								updateSelected((segment) => ({
									...segment,
									amount: clampZoomAmount(amount),
								}))
							}
						>
							{formatZoomAmount(amount)}
						</button>
					)}
				</For>
			</div>
			...
			<div class="space-y-2">
				<div class="flex items-center justify-between text-xs text-gray-11">
					<span>Shared focal point</span>
					<button
						type="button"
						class="rounded-md px-2 py-1 text-gray-10 hover:bg-gray-4 hover:text-gray-12"
						onClick={() => {
							const center = { x: 0.5, y: 0.5 };
							setBatchFocus(center);
							updateSelected((segment) => ({
								...segment,
								mode: { manual: center },
							}));
			...
			<div class="grid grid-cols-2 gap-1.5">
				<For each={["smooth", "instant"] as const}>
					{(style) => (
						<button
							type="button"
							class="rounded-lg border border-gray-3 bg-gray-1 px-2 py-2 text-xs capitalize text-gray-11 transition-colors hover:text-gray-12"
							onClick={() =>
								updateSelected((segment) => ({
									...segment,
									instantAnimation: style === "instant",
								}))
							}
						>
							{style} transition
			...
			<div class="space-y-2">
				<div class="flex justify-between text-xs text-gray-11">
					<span>Edge-safe margin</span>
					<span>{Math.round(EDGE_SAFE_MARGIN_DEFAULT * 100)}% default</span>
				</div>
				<Slider
					value={[EDGE_SAFE_MARGIN_DEFAULT * 100]}
					onChange={([percent]) =>
						updateSelected((segment) => ({
							...segment,
							edgeSnapRatio: clampEdgeSafeMargin(percent / 100),
						}))
					}
					minValue={0}
					maxValue={EDGE_SAFE_MARGIN_MAX * 100}
					step={1}
					formatTooltip="%"
				/>
			</div>

			<div class="flex flex-wrap gap-1.5 border-t border-gray-3 pt-3">
				<button
					type="button"
					class="rounded-lg px-2.5 py-1.5 text-xs text-gray-11 hover:bg-gray-4 hover:text-gray-12"
					onClick={() => {
						const first = props.segments[0]?.segment;
						if (!first) return;
						copyZoomSettings(first);
						toast.success("Zoom style copied");
					}}
				>
					Copy first
				</button>
				...
				<button
					type="button"
					class="ml-auto rounded-lg px-2.5 py-1.5 text-xs text-gray-11 hover:bg-gray-4 hover:text-gray-12"
					onClick={() => {
						updateSelected(resetZoomSettings);
						toast.success(`${props.segments.length} zooms reset`);
					}}
				>
					Reset all
				</button>
...
function ZoomSegmentConfig(props: {
	...
	const replaceSegment = (next: ZoomSegment) =>
		setProject("timeline", "zoomSegments", props.segmentIndex, next);
	...
	const pasteSettings = () => {
		...
		replaceSegment(applyZoomSettings(props.segment, copied));
		toast.success("Zoom style pasted");
	};

	return (
		<>
			<div class="flex flex-wrap gap-1.5 rounded-xl border border-gray-3 bg-gray-2/70 p-2">
				<button
					type="button"
					class="rounded-lg px-2.5 py-1.5 text-xs text-gray-11 transition-colors hover:bg-gray-4 hover:text-gray-12"
					onClick={() => {
						copyZoomSettings(props.segment);
						toast.success("Zoom style copied");
					}}
				>
					Copy style
				...
				<button
					type="button"
					class="ml-auto rounded-lg px-2.5 py-1.5 text-xs text-gray-11 transition-colors hover:bg-gray-4 hover:text-gray-12"
					onClick={() => {
						replaceSegment(resetZoomSettings(props.segment));
						toast.success("Zoom reset to automatic defaults");
					}}
				>
					Reset
				</button>
			...
			>
				<div class="space-y-3">
					<div class="grid grid-cols-6 gap-1.5">
						<For each={ZOOM_AMOUNT_PRESETS}>
							{(amount) => (
								<button
									type="button"
									aria-pressed={Math.abs(props.segment.amount - amount) < 0.001}
									class="rounded-lg border border-gray-3 bg-gray-2 px-1 py-1.5 text-[11px] text-gray-11 transition-colors hover:border-gray-6 hover:text-gray-12 aria-pressed:border-blue-9 aria-pressed:bg-blue-3 aria-pressed:text-blue-11"
					...
					<Slider
						value={[clampZoomAmount(props.segment.amount)]}
						onChange={([amount]) => updateAmount(amount)}
						minValue={ZOOM_AMOUNT_MIN}
						maxValue={ZOOM_AMOUNT_MAX}
						step={0.01}
						formatTooltip="x"
					/>
					<label class="flex items-center gap-2 text-xs text-gray-11">
						...
						<Input
							...
							value={clampZoomAmount(props.segment.amount)}
							onChange={(event) => {
								const amount = parseZoomAmount(event.currentTarget.value);
								if (amount !== null) updateAmount(amount);
							}}
							class="h-8 py-2 text-right"
							aria-label="Custom zoom amount"
						/>
						<span>x</span>
					</label>
				</div>
			</Field>
			<Field name="Transition" icon={<IconLucideWind class="size-4" />}>
				<div class="grid grid-cols-2 gap-1.5">
					<For each={["smooth", "instant"] as const}>
						{(style) => (
							<button
								...
								aria-pressed={
									style ===
									(props.segment.instantAnimation ? "instant" : "smooth")
								}
								class="rounded-lg border border-gray-3 bg-gray-2 px-2 py-2 text-xs capitalize text-gray-11 transition-colors hover:text-gray-12 aria-pressed:border-blue-9 aria-pressed:bg-blue-3 aria-pressed:text-blue-11"
								onClick={() =>
									setProject(
										"timeline",
										"zoomSegments",
			...
			<Field name="Edge-safe margin" icon={<IconLucideBoxSelect />}>
				<div class="space-y-2">
					<Slider
						value={[
							clampEdgeSafeMargin(
								props.segment.edgeSnapRatio ?? EDGE_SAFE_MARGIN_DEFAULT,
							) * 100,
						]}
						onChange={([percent]) =>
							setProject(
								...
								props.segmentIndex,
								"edgeSnapRatio",
								clampEdgeSafeMargin(percent / 100),
							)
						}
						minValue={0}
						maxValue={EDGE_SAFE_MARGIN_MAX * 100}
						...
						formatTooltip="%"
					/>
					<p class="text-[11px] leading-4 text-gray-10">
						Keeps automatic focal points away from clipped frame edges.
					</p>
				</div>
			</Field>
		...
		<>
			...
			<div class="space-y-0.5 pt-2">
				...
				<Show when={offsetsAutoCalculated()}>
					<p class="text-gray-11">
						FlowReco calculated these offsets automatically to keep audio in
