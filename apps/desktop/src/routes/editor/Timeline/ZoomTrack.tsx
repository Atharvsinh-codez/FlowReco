...
import { preserveZoomsOnEmptyGeneration } from "../zoom-controls";
import {
	useSegmentContext,
	useTimelineContext,
	useTrackContext,
...
export function ZoomTrack(props: {
	...
	const handleGenerateZoomSegments = async () => {
		if (isGeneratingAutoZoom()) return;

		const current = [...(project.timeline?.zoomSegments ?? [])];
		const wasRegeneration = current.length > 0;
		setIsGeneratingAutoZoom(true);
		try {
			const generated = await commands.generateZoomSegmentsFromClicks();
			const result = preserveZoomsOnEmptyGeneration(current, generated);
			if (!result.replaced) {
				toast("No new interaction zooms found. Existing zooms were kept.");
				return;
			}

			setProject("timeline", "zoomSegments", result.segments);
			toast.success(
				`${result.segments.length} automatic zoom${
					result.segments.length === 1 ? "" : "s"
				} ${wasRegeneration ? "regenerated" : "generated"}`,
			);
		} catch (error) {
			console.error("Failed to generate zoom segments:", error);
			toast.error(
				"Automatic zoom generation failed. Existing zooms were kept.",
			);
		} finally {
	...
	return (
		...
		>
			...
			>
				<Show when={hasRecordedCursorData()}>
					<div
						class="absolute right-2 top-1 z-20"
						onMouseDown={(event) => event.stopPropagation()}
					>
						<Button
							variant="gray"
							size="sm"
							class="border border-gray-6 bg-gray-2/95 shadow-sm"
							disabled={isGeneratingAutoZoom()}
							onClick={() => void handleGenerateZoomSegments()}
						>
							{isGeneratingAutoZoom() ? "Regenerating…" : "Regenerate"}
						</Button>
					</div>
				</Show>
