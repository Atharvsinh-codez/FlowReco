		...
		try {
			...
			if (!update) {
				await dialog.message(
					"You're already using the latest available FlowReco build.",
					{
			...
			const shouldUpdate = await dialog.confirm(
				`Version ${update.version} of FlowReco is available, would you like to install it?`,
				{ title: "Update FlowReco", okLabel: "Update", cancelLabel: "Ignore" },
			);

			if (shouldUpdate) navigate("/update");
		} catch (e) {
			console.error("Failed to check for updates:", e);
			const openReleases = await dialog
				.confirm(
					"Couldn't check for updates automatically. Review published builds on the FlowReco releases page. Your local projects won't be changed.",
					{
						title: "Update FlowReco",
						okLabel: "View releases",
						cancelLabel: "Later",
					},
				)
				.catch(() => false);
			if (openReleases)
				await shell.open(
					"https://github.com/Atharvsinh-codez/FlowReco/releases",
				);
		} finally {
