			...
			<Show when={updateError()}>
				<div class="flex flex-col gap-4 items-center text-center max-w-md">
					...
					<p class="text-(--text-tertiary)">
						Download a verified build from the FlowReco GitHub Releases page.
						Your local projects will not be changed.
					</p>
					<p class="text-(--text-tertiary) text-xs">
						If this issue persists, open an issue in the FlowReco repository.
					</p>
				...
				{(_update) => {
					...
					return (
						<div>
							<Switch
								fallback={
									<IconLucideLoaderCircle class="animate-spin size-4 text-(--text-primary)" />
								}
							>
								<Match when={updateStatus()?.type === "done"}>
									<div class="flex flex-col gap-4 items-center">
										<p class="text-(--text-tertiary)">
											Update has been installed. Restart FlowReco to finish
											updating.
										</p>
