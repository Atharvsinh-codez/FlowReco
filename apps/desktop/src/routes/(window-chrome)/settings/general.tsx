...
import { applyStoredTelemetryPreference } from "~/utils/analytics";
...
function AppearanceSection(props: {
	...
	return (
		<Section
			title="Appearance"
			description="Match FlowReco to your system theme or pick a fixed look."
		>
			<SectionCard padded>
				<div
	...
	const handleChange = async <K extends keyof typeof settings>(
		...
		try {
			...
			if (key === "enableTelemetry") {
				await applyStoredTelemetryPreference();
			}
		} catch (error) {
		...
		>
			<SettingsPageContent>
				...
				{ostype === "macos" && (
					<Section
						title="App"
						description="Choose how FlowReco shows up on your system."
					>
						<SectionRows>
							<ToggleSettingItem
								label="Always show dock icon"
								description="Keep FlowReco in the dock even when no windows are open."
								value={!settings.hideDockIcon}
								onChange={(v) => handleChange("hideDockIcon", !v)}
							/>
							<ToggleSettingItem
								label="System notifications"
								description="Show notifications for clipboard copies, saved files, and more. You may need to allow FlowReco in your system's notification settings."
				...
				>
					<SectionRows>
						...
						/>
						<ToggleSettingItem
							label="Capture exact keyboard presses"
							description="Stores exact key names and timing locally for keyboard overlays. This may reveal sensitive input, so leave it off when entering passwords or other private information."
...
function StorageSection(props: {
	...
	return (
		<Section
			title="Storage"
			description="Where FlowReco saves your recordings."
		>
			<SectionCard padded>
		...
		<Section title="Privacy">
			<SectionRows>
				<ToggleSettingItem
					...
					description="Optional: share anonymous reliability events with FlowReco. Recording contents, window titles, file paths, and exact keyboard events are never included. Nothing is sent until you turn this on."
					value={props.value}
...
function UpdatesSection(props: {
	...
	return (
		<Section
			title="Updates"
			description="Choose which FlowReco builds you receive."
		>
			<SectionCard>
					...
					<div class="flex justify-between items-start gap-4">
						<div class="flex flex-col gap-0.5 min-w-0">
							...
							<p class="text-xs leading-snug text-gray-10">
								Which release channel FlowReco updates from.
							</p>
						</div>
						<SegmentedControl
							value={props.value}
...
function ServerURLSetting(props: {
	...
	return (
		<Section
			title="Self-host"
			description="Only change this if you are running your own instance of FlowReco Web."
		>
			<SectionCard padded>
				<div class="flex flex-col gap-3">
					<label class="flex flex-col gap-1.5">
						<span class="text-[13px] text-gray-12">FlowReco Server URL</span>
						<Input
							class="bg-gray-3"
							value={value()}
...
function ExcludedWindowsCard(props: {
	...
	return (
		<Section
			...
			description={
				props.isWindows
					? "Hide windows from recordings. On Windows, only FlowReco-related windows can be excluded."
						...
						<div class="flex items-start gap-2">
							...
							<div class="min-w-0 flex-1 space-y-1">
								<p class="text-xs font-medium text-amber-11">
									Recommended FlowReco windows are not excluded
								</p>
