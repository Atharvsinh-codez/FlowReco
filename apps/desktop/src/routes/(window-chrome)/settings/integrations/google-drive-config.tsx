	...
	const connect = useMutation(() => ({
		mutationFn: async () => {
			...
			if (response.status === 403) {
				await commands.globalMessageDialog(
					"This FlowReco server does not allow Google Drive storage. Ask the server administrator to enable it.",
				);
				return null;
			}
	...
	return (
		<div class="cap-settings-page flex flex-col h-full custom-scroll">
			<SettingsPageContent>
				...
				<Section
					title="Connection"
					description="Google Drive stores new uploads in a private application folder. Existing server-hosted and S3 videos keep using their current storage."
				>
					<SectionCard padded class="custom-scroll">
						<Suspense
							fallback={
								<div class="flex justify-center items-center w-full h-screen">
									<img src={RecoMark} alt="" class="animate-spin size-16" />
								</div>
							}
						>
