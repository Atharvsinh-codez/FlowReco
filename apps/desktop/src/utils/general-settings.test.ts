...
describe("general-settings", () => {
	it("normalizes transcription hints from text input", () => {
		expect(
			parseTranscriptionHints(" FlowReco \n\nTypeScript\nFlowReco\nGitHub\n"),
		).toEqual(["FlowReco", "TypeScript", "GitHub"]);
	});

	it("formats transcription hints for the textarea", () => {
		expect(
			formatTranscriptionHints(["FlowReco", "TypeScript", "FlowReco"]),
		).toBe("FlowReco\nTypeScript");
	});
	...
	it("preserves explicit privacy opt-ins", () => {
		expect(
			deriveGeneralSettings({
				enableNativeCameraPreview: false,
