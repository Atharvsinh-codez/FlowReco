export const recordScreenMacContent = {
	...
	heroTLDR:
		"macOS won't record internal audio on its own — Cmd+Shift+5 and QuickTime only capture your mic. The quickest fix is FlowReco: toggle on system audio, hit record, and you're capturing screen and sound in two clicks. No BlackHole, no Terminal, free.",
	...
	methods: [
		{
			title: "Method 1: Record Mac Screen + Audio With FlowReco (2 Clicks)",
			description:
				"FlowReco captures internal audio on macOS natively, so there's nothing to route and nothing to install beyond the app itself.",
			steps: [
				{
					title: "Step 1: Install FlowReco",
					content:
						'Download FlowReco from <a href="/download">cap.so/download</a> and drag it into Applications. The first time you record, macOS asks for Screen Recording and Microphone permissions in System Settings → Privacy & Security — grant both so FlowReco can capture your display and audio.',
				},
				{
					title: "Step 2: Turn on system audio and record",
					content: `
            <ol>
              <li>Click the FlowReco icon in your menu bar.</li>
            ...
            <p>That's the whole setup. FlowReco records everything coming out of your speakers — app sounds, a video playing in your browser, a call — alongside your voice, with no BlackHole and no Audio MIDI Setup.</p>
            <p><strong>Good to know:</strong> FlowReco runs without kernel extensions and works on the latest macOS releases and Apple Silicon (M1 through M4). If you only want a landing page for this workflow, the <a href="/mac-screen-recording-with-audio">Mac screen recorder with internal audio</a> page covers it too.</p>
          `,
				},
				{
					title: "Step 3: Trim, level, and share",
					content: `
            <p>When you stop, FlowReco hands you a shareable link right away, or you can open the recording in the editor first:</p>
            <ul>
		...
		},
		{
            ...
            <p>The catch: BlackHole routes system audio into the "microphone" slot, so recording your voice <strong>and</strong> system sound together means stacking an Aggregate Device on top of all this — the multi-step dance FlowReco skips entirely. <a href="/screen-recorder-mac">See how FlowReco compares as a Mac screen recorder</a>.</p>
          `,
				},
			],
		},
		...
		items: [
			{
				...
				answer:
					"With FlowReco, make sure the menu-bar control is set to \"Record System Audio\" before you start — it's off by default. With QuickTime or OBS, this almost always means your system output isn't pointed at the Multi-Output Device that includes BlackHole.",
			},
			{
				question: "I hear an echo or doubled audio",
				answer:
					"That happens when your mic is picking up the same sound your speakers are playing. Use headphones, or in FlowReco turn the microphone off and keep only system audio if you don't need narration.",
			},
			{
				question: "My microphone is being recorded but app sound isn't",
				answer:
					"FlowReco records both at once — toggle System Audio and Microphone on in the recording menu. If you're on QuickTime, it can only listen to one input, which is why capturing voice and system sound together needs an Aggregate Device.",
			},
			{
				question: "FaceTime or Zoom audio isn't captured",
				answer:
					'Some apps apply extra privacy protection to their audio. In FlowReco, choose "Record entire screen" rather than a single window to capture protected app audio reliably.',
			},
			{
				question: "System audio recording broke after a macOS update",
				answer:
					"Major macOS updates can reset screen recording permissions. Open System Settings → Privacy & Security → Screen Recording and confirm FlowReco is still enabled, then relaunch it.",
			},
		],
	},
	...
	proTips: {
		...
		tips: [
			...
			},
			{
				...
				description:
					"If you're demoing animations, transitions, or anything that scrolls fast, bump the frame rate up in FlowReco's settings — 60 FPS keeps it smooth.",
			},
			{
				title: "Let captions do the work",
				description:
					"FlowReco can auto-generate captions from the audio it just recorded, which makes tutorials watchable on mute and more accessible without extra effort.",
			},
			{
	...
	faqs: [
		{
			question: "Can FlowReco record internal audio on a Mac without BlackHole?",
			answer:
				'Yes. FlowReco captures macOS system audio natively, so you don\'t need BlackHole, Loopback, or any virtual audio driver. Turn on "Record System Audio" and everything playing through your Mac is captured with the screen.',
		},
		{
			question: "Why can't QuickTime or Cmd+Shift+5 record system audio?",
			answer:
				"Apple restricts internal audio capture on macOS for privacy and copyright reasons, so the built-in tools only record your microphone. To capture the sound your apps are playing, you need either a virtual audio device or an app like FlowReco that handles it natively.",
		},
		{
			question: "Does this work on M1, M2, M3, and M4 Macs?",
			answer:
				"Yes. FlowReco is built for Apple Silicon and runs natively on every M-series chip, as well as the latest versions of macOS.",
		},
		{
			question: "Is there a time limit on recordings?",
			answer:
				"FlowReco's Studio Mode records locally with no time limit — record for as long as you need and export the full length. On the free plan, Instant Mode (the instantly shareable links) is capped at 5 minutes per recording; upgrading lifts that cap.",
		},
		{
			question: "Can I record system audio and my microphone at the same time?",
			answer:
				"Yes, and FlowReco keeps them on separate tracks so you can adjust each one in the editor. That's ideal for narrating a tutorial while the app you're demoing still makes its own sounds.",
		},
		{
			question: "Can I record the audio from just one app?",
			answer:
				"macOS doesn't expose a per-app audio API, so FlowReco records all system audio together. The simplest workaround is to quit noisy background apps before recording, or trim unwanted sections out afterward.",
		},
		{
			question: "Will recording system audio slow my Mac down?",
			answer:
				"Not noticeably. FlowReco is lightweight and built natively for macOS, so even recording a high-resolution screen with audio leaves plenty of headroom on a modern Mac.",
		},
	],
	cta: {
		title: "Record Your Mac Screen With Audio — Free",
		description:
			"Skip the BlackHole setup. FlowReco captures system sound and your mic on macOS natively, then hands you a shareable link the moment you stop.",
		buttonText: "Download FlowReco",
		buttonLink: "/download",
		subtitle:
