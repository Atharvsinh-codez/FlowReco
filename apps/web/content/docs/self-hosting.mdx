---
title: "Self-hosting FlowReco Web"
summary: "How to deploy FlowReco Web on your own infrastructure"
tags: "Deployment, Self-hosting"
image: "/docs/self-hosting.webp"
---

FlowReco Web is the optional web application for publishing and sharing recordings. You run it at a server origin you control.
You can upload videos to it from the dashboard or from FlowReco Desktop.

## Quick Start (One Command)

The fastest way to self-host FlowReco is with Docker Compose. This single command will start everything you need:

```bash
git clone https://github.com/Atharvsinh-codez/FlowReco.git
cd FlowReco
docker compose up -d
```

That's it! FlowReco will be available at `http://localhost:3000`.
...
3. Access FlowReco at `http://localhost:3000`
...
2. Point to the FlowReco repository
...
## Connecting FlowReco Desktop

1. Open FlowReco Desktop settings
2. Set 'FlowReco Server URL' to your deployment URL (e.g., `https://cap.yourdomain.com`)
...
The default `docker-compose.yml` contains **hardcoded placeholder secrets** that are visible in the public repository. Anyone who knows you're using FlowReco with defaults could potentially:
...
This is fine for local development or testing on a private network, but **you must generate unique secrets before exposing FlowReco to the internet**.
</Warning>
...
See [`packages/env/server.ts`](https://github.com/Atharvsinh-codez/FlowReco/blob/main/packages/env/server.ts) for all available options.

## Troubleshooting

**Can't access FlowReco after starting:**
```bash
docker compose logs cap-web
```
...
- [Open a GitHub issue](https://github.com/Atharvsinh-codez/FlowReco/issues/new)
