---
title: "FlowReco Apps: S3 Config (Cloudflare R2)"
summary: "Connect your own Cloudflare R2 bucket to FlowReco Desktop and maintain 100% ownership over your data."
tags: FlowReco Apps, Cloudflare R2, S3-compatible storage, Cloud storage configuration
image: "/docs/s3-config/cloudflare-r2.jpg"
---

# How to Connect Your Cloudflare R2 Bucket to FlowReco Desktop

Looking to take full control of your data storage within FlowReco? Integrating your Cloudflare R2 bucket gives you ownership, flexible scalability, and reduced latency, all while keeping your workflow smooth and secure. In this guide, you'll learn how to quickly set up and configure Cloudflare R2 with FlowReco Desktop—no complicated steps or robotic instructions, just straightforward guidance.

> **TL;DR**:
>
> 1. Open FlowReco Desktop → FlowReco Apps → S3 Config → Configure
...
> **Note**: While this tutorial focuses on Cloudflare R2, FlowReco Desktop supports any S3-compatible storage service. If you use another provider, just substitute the endpoint and credentials accordingly.

## Step 1: Open FlowReco Desktop and Access the FlowReco Apps Tab

1. **Open the FlowReco Desktop App**: Click the FlowReco icon on your desktop or launch it from your applications folder.
2. **Go to FlowReco Apps**: Once the app is open, look for the "FlowReco Apps" tab in the Settings menu.

## Step 2: Choose S3 Config and Start Configuration

1. **Click on S3 Config**: Within the FlowReco Apps section, locate "S3 Config."
...
Before connecting to FlowReco, ensure you have a Cloudflare R2 bucket ready:
...
4. **Set CORS Rules (Optional)**:
   ...
   - Add rules that allow `GET`, `HEAD`, and `PUT` from domains like `https://your-server.example`, `https://your-server.example`, `https://your-server.example`, and `https://your-server.example`.
   - This ensures that assets load smoothly if accessed directly from a browser.

## Step 4: Enter Your Cloudflare R2 Details in FlowReco Desktop

1. Return to the "R2 Config" section in FlowReco Desktop.
...
1. **Test Connection**: Click "Test Connection" in FlowReco Desktop to verify that it can communicate with your R2 bucket successfully.
2. **Save Configuration**: Once you see a successful connection message, save your settings.

With these steps completed, FlowReco Desktop will now use your Cloudflare R2 bucket to store and serve assets, giving you full control over how and where your content is hosted.

---

## Benefits of Using Cloudflare R2 with FlowReco Desktop

- **Full Data Ownership**: Your files stay under your control, not locked into a third-party platform.
- **S3-Compatible & Scalable**: R2 is designed to be S3-compatible, making it simple to use with FlowReco or other tools.
...
## Accessing Your R2 Files in FlowReco

After setting up Cloudflare R2 in FlowReco Desktop, any new shareable links you create will be served from your R2 bucket. This means:
...
With your Cloudflare R2 integration complete, you can enjoy seamless, secure, and efficient content delivery directly within FlowReco Desktop—no extra steps, no compromises.
