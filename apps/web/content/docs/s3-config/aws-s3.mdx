---
title: "FlowReco Apps: S3 Config (AWS)"
summary: "Connect your own S3 bucket to FlowReco and maintain 100% ownership over your data."
tags: "FlowReco Apps"
image: "/docs/s3-config/aws-s3.jpg"
---

# Connecting Your Amazon S3 Bucket to FlowReco Desktop

> **TL;DR**: Open FlowReco Desktop → FlowReco Apps → S3 Config → Configure → Select Amazon S3 → Enter your AWS credentials → Configure bucket permissions → Save and test with a shareable link upload.
>
> **Note**: While this guide focuses on AWS S3, FlowReco supports any S3-compatible storage service (like Supabase, MinIO, DigitalOcean Spaces, or Backblaze B2). The setup process is similar - you'll just need the appropriate endpoint URL and credentials from your provider.

## Step 1: Launch FlowReco Desktop and Access FlowReco Apps

1. **Open the FlowReco Desktop App**: Click the FlowReco icon on your desktop or find it in your applications folder to launch the app.

2. **Navigate to FlowReco Apps**: Once the app is open, locate the 'FlowReco Apps' tab in Settings.
...
## Step 4: Configure FlowReco Desktop

1. Open FlowReco Desktop and navigate to S3 Config
...
1. Test the Connection: Click on the 'Test Connection' button to verify that FlowReco can successfully connect to your S3 bucket.
2. Save the Configuration: If the test is successful, save the connection settings. Your Amazon S3 bucket is now integrated with FlowReco Desktop.

## Accessing Your S3 Files in FlowReco
