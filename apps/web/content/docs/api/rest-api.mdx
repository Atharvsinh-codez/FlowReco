---
title: "REST API"
summary: "Programmatic access to FlowReco via the REST API"
tags: "API, Integration"
---

The FlowReco Developer API provides programmatic access to videos created through your developer apps. It is designed for server-to-server integrations, automated workflows, and building custom recording experiences on top of FlowReco.
...
1. Go to [your FlowReco server dashboard](/dashboard/developers).
...
```
https://your-server.example/api/developer/v1
```

**SDK API (public key):**

```
https://your-server.example/api/developer/sdk/v1
```

If you are self-hosting FlowReco, replace `your server origin` with your custom domain.
...
```bash
curl https://your-server.example/api/developer/v1/videos?limit=10 \
...
```bash
curl https://your-server.example/api/developer/v1/videos/abc123def456 \
  -H "Authorization: Bearer csk_your_secret_key_here"
```
...
```bash
curl https://your-server.example/api/developer/v1/videos/abc123def456/status \
  -H "Authorization: Bearer csk_your_secret_key_here"
```
...
```bash
curl -X DELETE https://your-server.example/api/developer/v1/videos/abc123def456 \
  -H "Authorization: Bearer csk_your_secret_key_here"
```
...
```bash
curl https://your-server.example/api/developer/v1/usage \
  -H "Authorization: Bearer csk_your_secret_key_here"
```
...
```bash
curl -X POST https://your-server.example/api/developer/sdk/v1/videos/create \
...
```json
{
  ...
  "s3Key": "developer/app_789/abc123def456/result.mp4",
  "shareUrl": "https://your-server.example/dev/abc123def456",
  "embedUrl": "https://your-server.example/embed/abc123def456?sdk=1"
}
```
...
FlowReco provides official SDK packages to simplify integration:

- **`@cap/sdk-recorder`** -- Recording SDK for capturing video in web applications. Includes React bindings.
- **`@cap/sdk-embed`** -- Embedding SDK for displaying FlowReco videos. Available for React and vanilla JavaScript.
