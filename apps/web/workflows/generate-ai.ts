...
const GENERATED_TITLE_PATTERN =
	/^(FlowReco (Recording|Upload) - .+|Untitled|\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}|.+ \((Display|Window|Area|Camera)\) \d{4}-\d{2}-\d{2} \d{2}:\d{2} [AP]M)$/;
...
async function generateSingleChunk(
	...
	const prompt = `You are FlowReco AI, an expert at analyzing video content and creating comprehensive summaries.
...
async function generateMultipleChunks(
	...
	for (let i = 0; i < chunks.length; i++) {
		...
		const chunkPrompt = `You are FlowReco AI, an expert at analyzing video content. This is section ${i + 1} of ${chunks.length} from a video that is ${videoDuration} seconds long (${Math.floor(videoDuration / 60)}:${String(Math.floor(videoDuration % 60)).padStart(2, "0")} total). This section covers timestamp ${Math.floor(chunk.startTime / 60)}:${String(chunk.startTime % 60).padStart(2, "0")} to ${Math.floor(chunk.endTime / 60)}:${String(chunk.endTime % 60).padStart(2, "0")}.
...
${chunk.text}`;
	...
	const finalPrompt = `You are FlowReco AI, an expert at synthesizing information into comprehensive, well-organized summaries.
