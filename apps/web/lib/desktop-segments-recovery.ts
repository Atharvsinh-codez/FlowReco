...
async function retireCandidate({
	...
	await db()
		...
		.set({
			...
			phase: "error",
			processingError: `${RECOVERY_ABANDONED_PREFIX} ${status} — the upload was interrupted and could not be recovered automatically. Reopen FlowReco on the recording device to retry, or record again.`,
		})
		.where(
			and(
				eq(videoUploads.videoId, videoId),
