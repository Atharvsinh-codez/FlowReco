...
describe("HipaaCompliantScreenRecordingPage FAQ schema", () => {
	const faqs = [
		{
			...
			answer:
				"FlowReco supports HIPAA-compliant workflows when configured with self-hosted storage.",
		},
		{
			question: "Does FlowReco store recordings on its own servers?",
			answer:
				"By default, FlowReco uploads recordings to FlowReco's cloud storage. However, FlowReco fully supports custom S3-compatible storage.",
		},
		{
			question:
				"Is FlowReco open source and auditable for HIPAA vendor assessments?",
			answer: "Yes. FlowReco is MIT-licensed and fully open source on GitHub.",
		},
		{
			question: "Can I disable AI transcription in FlowReco for HIPAA compliance?",
			answer:
				"Yes. FlowReco's AI auto-captions are optional and can be disabled entirely in settings.",
		},
		{
			question: "Does FlowReco support AWS S3 for HIPAA-eligible storage?",
			answer:
				"Yes. FlowReco supports AWS S3 as a storage backend. AWS S3 is HIPAA-eligible when covered by an AWS Business Associate Agreement (BAA).",
		},
		{
			question: "Can the entire FlowReco platform be self-hosted?",
			answer:
				"Yes. The complete FlowReco platform can be self-hosted on your own infrastructure.",
		},
		{
			question:
				"How do I restrict access to HIPAA screen recordings shared via FlowReco?",
			answer: "FlowReco supports password-protected sharing links.",
		},
		{
	...
	it("maps each FAQ to a Question entity with acceptedAnswer", () => {
		...
		expect(schema.mainEntity[0]).toEqual({
			"@type": "Question",
			name: "Can FlowReco be used for HIPAA-compliant screen recording?",
			acceptedAnswer: {
				"@type": "Answer",
				text: faqs[0]?.answer,
			},
