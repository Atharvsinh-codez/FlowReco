...
describe("createHowToSchema", () => {
			...
			steps: [
				{
					...
					text: "Download FlowReco for free from cap.so/download.",
				},
				{
	...
	it("applies schema to how-to-screen-record page steps", () => {
		const howToSteps = [
			{
				name: "Download and install FlowReco",
				text: "Download FlowReco for free from cap.so/download for Mac or Windows.",
			},
			{
				name: "Choose your recording settings",
				text: "Open FlowReco and select your recording source.",
			},
			{
		...
		const schema = createHowToSchema({
			...
			description:
				"Learn how to screen record with audio on Mac, Windows, or in your browser using FlowReco.",
			totalTime: "PT2M",
			steps: howToSteps,
		});
		...
		expect(schema.step[0]?.name).toBe("Download and install FlowReco");
