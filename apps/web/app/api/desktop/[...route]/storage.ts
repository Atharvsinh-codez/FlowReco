...
const htmlResponse = ({
	...
	const action = redirectUrl
		? `<div class="actions"><a class="button" href="${escapeHtml(redirectUrl)}">${escapeHtml(redirectLabel ?? "Back to FlowReco")}</a></div>`
		: "";
		...
		if (error) {
			return c.html(
				htmlResponse({
					...
					body: "You can close this window and try again from FlowReco settings.",
				}),
				400,
			);
		}
			...
			htmlResponse(
				organizationId
					? {
							...
							body: 'Your Google account is now linked to FlowReco. We\'ve created a "FlowReco" folder in your Drive to store your recordings.',
					...
					: {
							title: "Google Drive connected",
							body: "Return to the FlowReco app to finish setting up your storage.",
						},
			),
		);
	} catch (error) {
		...
		return c.html(
			htmlResponse(
				organizationIdForRedirect
					? {
							title: "Google Drive was not connected",
							body: "Something went wrong while linking your Google account. You can try again from FlowReco settings.",
							redirectUrl: orgRedirectUrl,
							redirectLabel: "Back to settings",
							redirectSeconds: 8,
						}
