...
export const Caps = ({
	...
	const { mutate: deleteCaps, isPending: isDeletingCaps } = useEffectMutation({
		mutationFn: Effect.fn(function* (ids: Video.VideoId[]) {
			...
			if (successCount > 0 && errorCount > 0) {
				...
				return yield* Effect.fail(
					new Error(
						`Couldn't delete ${errorCount} recording${errorCount === 1 ? "" : "s"}. Try again.`,
					),
				);
			}
		}),
		...
		onSuccess: (data: { success: number; error?: number }) => {
			...
			if (data.error) {
				toast.success(
					`Deleted ${data.success} recording${
						data.success === 1 ? "" : "s"
				...
				toast.success(
					`Deleted ${data.success} recording${data.success === 1 ? "" : "s"}`,
				);
			}
		},
		onError: (error: unknown) => {
			const message =
				error instanceof Error
					? error.message
					: "Couldn't delete the recordings. Try again.";
			toast.error(message);
		},
	});
	...
	const { mutate: deleteCap, isPending: isDeletingCap } = useEffectMutation({
		...
		onError: (_error: unknown) =>
			toast.error("Couldn't delete the recording. Try again."),
	});
