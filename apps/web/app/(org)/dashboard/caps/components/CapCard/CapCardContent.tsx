...
export const CapCardContent: React.FC<CapContentProps> = ({
	...
	const handleDateBlur = async () => {
		...
		try {
			...
			toast.success("Recording date updated");
			setIsDateEditing(false);
			router.refresh();
		} catch (error) {
			if (error instanceof Error) {
		...
		<div>
			<div className="h-[1.25rem] mb-1">
				...
				{isEditing && !sharedCapCard ? (
					...
					<button
						type="button"
						className="text-md text-left w-full truncate leading-[1.25rem] text-gray-12 font-medium p-0 m-0 h-[1.25rem] tracking-normal"
						onClick={() => {
							if (!sharedCapCard) {
								if (userId === cap.ownerId) {
									setIsEditing(true);
