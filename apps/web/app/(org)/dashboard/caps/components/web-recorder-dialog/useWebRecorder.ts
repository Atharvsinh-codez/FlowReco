...
const getStartRecordingErrorMessage = (
	...
	if (typeof DOMException !== "undefined" && error instanceof DOMException) {
		...
		if (error.name === "InvalidStateError") {
			return "Your browser blocked the capture request because this tab wasn't active. Bring FlowReco to the foreground and click Start recording again.";
		}
	...
	if (error instanceof TypeError) {
		if (/fetch|network|load failed|failed to fetch/i.test(error.message)) {
			return "FlowReco couldn't create the upload session. Check Chrome extensions, privacy settings, or network access, then try again.";
		}
