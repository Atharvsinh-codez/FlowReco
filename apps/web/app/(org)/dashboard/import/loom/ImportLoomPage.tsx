...
export const ImportLoomPage = () => {
	...
	const [upgradeModalOpen, setUpgradeModalOpen] = useState(
		requiresHostedPlanUpgrade(buildEnv.NEXT_PUBLIC_IS_CAP, user?.isPro),
	);
	...
	const handleSingleImport = async () => {
		...
		if (requiresHostedPlanUpgrade(buildEnv.NEXT_PUBLIC_IS_CAP, user.isPro)) {
		...
		try {
			...
			toast.success(
				"Loom import started. The recording will appear in your library shortly.",
			);
	...
	const handleTemplateDownload = () => {
		...
		link.href = url;
		link.download = "reco-loom-import-template.csv";
		link.click();
				...
				<div className="flex gap-4 items-start">
					...
					<div>
						...
						<p className="mt-1 max-w-xl text-sm text-gray-10">
							{canUseCsvImport
								? "Bring a single Loom video into FlowReco, or bulk import recordings for organization members and new users from a CSV."
								: "Paste a Loom share link to bring it into FlowReco."}
						</p>
					</div>
				</div>
			</div>
										...
										<div className="flex gap-3 items-start p-4 rounded-lg border bg-red-2 border-red-4 text-red-11">
											...
											<div className="text-sm">
												...
												<p className="mt-1 text-red-10">
													Split this file into smaller batches. Server
													administrators can raise this limit if needed.
												</p>
											</div>
										</div>
									)}
