...
export async function generateMetadata(
	...
	return Effect.flatMap(Videos, (v) => v.getByIdForViewing(videoId)).pipe(
		Effect.map(
			Option.match({
				onNone: () =>
					awaitRecording
						? {
								title: "FlowReco: Preparing Video",
				...
				onSome: ([video]) => {
					...
					return {
						title: `${video.name} | FlowReco Recording`,
						description: "Watch this video on FlowReco",
						openGraph: {
							images: [
								{
									url: previewImageUrl,
		...
		Effect.catchTags({
			PolicyDenied: () =>
				Effect.succeed({
					title: "FlowReco: This video is restricted",
			...
			VerifyVideoPasswordError: () =>
				Effect.succeed({
					title: "FlowReco: Password Protected Video",
