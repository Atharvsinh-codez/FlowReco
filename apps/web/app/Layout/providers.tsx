...
export function PostHogProvider({
	...
	const telemetryEnabled = isTelemetryEnabled(
		buildEnv.NEXT_PUBLIC_TELEMETRY_ENABLED,
	);
	...
	if (!telemetryEnabled || !key || !host || !options) {
		if (
			telemetryEnabled &&
...
function CapDevtools() {
	return (
		<div className="flex flex-col p-4 space-y-4">
			<h1 className="text-2xl font-semibold">FlowReco Devtools</h1>
			<div className="space-y-2">
				<h1 className="text-lg font-semibold">Hosted access</h1>
				<p className="text-xs text-muted-foreground">
					Toggle the current user's hosted access (dev only)
				</p>
