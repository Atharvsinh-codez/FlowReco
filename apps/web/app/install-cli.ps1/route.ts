...
function Find-CapAppPath {
	$candidates = @(
		"$env:LOCALAPPDATA\Programs\FlowReco\FlowReco.exe",
		"$env:LOCALAPPDATA\FlowReco\FlowReco.exe",
		"$env:ProgramFiles\FlowReco\FlowReco.exe",
		"${programFilesX86}\FlowReco\FlowReco.exe"
	)
...
function Install-CapDesktop {
	...
	$installerPath = Join-Path ([System.IO.Path]::GetTempPath()) ("FlowReco-" + [System.Guid]::NewGuid().ToString("N") + ".exe")

	try {
		Write-Host "Downloading FlowReco Desktop..."
		Invoke-WebRequest -UseBasicParsing -Uri $downloadUrl -OutFile $installerPath
		Write-Host "Installing FlowReco Desktop..."
		$process = Start-Process -FilePath $installerPath -ArgumentList "/S" -Wait -PassThru

		if ($process.ExitCode -ne 0) {
			Write-Error "FlowReco Desktop installer failed with exit code $($process.ExitCode)."
			exit 1
		}
	} finally {
...
if (-not $appPath) {
	...
	Write-Error "FlowReco Desktop was not found at $appPath."
	exit 1
}
...
if (-not $appPath) {
	...
	if (-not $appPath) {
		Write-Error "FlowReco Desktop was installed, but FlowReco.exe was not found. Open FlowReco Desktop once, then run this script again."
		exit 1
	}
}
...
if (-not (Test-Path $cliTarget)) {
	Write-Host "This FlowReco Desktop install does not include the CLI. Reinstalling FlowReco Desktop..."
	Install-CapDesktop
	$appPath = Find-CapAppPath
	...
	if (-not (Test-Path $cliTarget)) {
		Write-Error "This FlowReco Desktop install does not include the CLI."
		exit 1
	}
}
...
if (Test-Path $shimPath) {
	...
	if (-not ($contents.Contains($cliTarget) -or $contents.Contains($shimTarget) -or $contents -match '\\FlowReco\\cap-cli\.exe')) {
		Write-Error "$shimPath already exists and is not managed by FlowReco. Remove it or set CAP_CLI_INSTALL_DIR, then run this script again."
		exit 1
	}
}
