...
find_cap_app_macos() {
	for candidate in "/Applications/FlowReco.app" "$HOME_DIR/Applications/FlowReco.app"; do
		if [ -d "$candidate" ]; then
			APP_PATH="$candidate"
			return 0
		fi
...
linux_require_supported_arch() {
	case "$(uname -m)" in
		x86_64|amd64)
			;;
		*)
			echo "FlowReco for Linux currently ships x86_64 desktop builds. Unsupported architecture: $(uname -m)" >&2
			exit 1
			;;
	esac
}
...
find_cli_in_dir() {
	...
	for dir in "$root" "$root/usr/bin" "$root/usr/lib/cap" "$root/usr/lib/FlowReco" "$root/usr/lib64/cap" "$root/opt/FlowReco" "$root/resources" "$root/Contents/MacOS"; do
...
find_linux_system_cli() {
	for root in /usr/bin /usr/local/bin /usr/lib/cap /usr/lib/FlowReco /usr/lib64/cap /opt/FlowReco /opt/cap /usr/share/cap /usr/share/FlowReco; do
...
install_cap_desktop_macos() {
	...
	APP_PATH="$APP_DIR/FlowReco.app"
	APP_TMP="$APP_DIR/.FlowReco.app.installing"
	TMP_ROOT="$(mktemp -d "$TMP_BASE/cap-cli-install.XXXXXX")"
	DMG_PATH="$TMP_ROOT/FlowReco.dmg"
	MOUNT_DIR="$TMP_ROOT/mount"
	mkdir -p "$APP_DIR" "$MOUNT_DIR"

	trap cleanup_desktop_install EXIT HUP INT TERM

	echo "Downloading FlowReco Desktop..."
	curl -fL "$DOWNLOAD_URL" -o "$DMG_PATH"
	hdiutil attach "$DMG_PATH" -nobrowse -quiet -mountpoint "$MOUNT_DIR"
	MOUNTED=1

	SOURCE_APP=""
	for candidate in "$MOUNT_DIR"/FlowReco.app "$MOUNT_DIR"/*.app; do
		if [ -d "$candidate" ] && [ "$(basename "$candidate")" = "FlowReco.app" ]; then
	...
	if [ -z "$SOURCE_APP" ]; then
		echo "Downloaded FlowReco Desktop image did not contain FlowReco.app." >&2
		exit 1
	fi

	echo "Installing FlowReco Desktop to $APP_PATH..."
	rm -rf "$APP_TMP"
...
install_cap_desktop_linux_deb() {
	linux_require_supported_arch
	TMP_ROOT="$(mktemp -d "$TMP_BASE/cap-cli-install.XXXXXX")"
	DEB_PATH="$TMP_ROOT/FlowReco.deb"
	trap cleanup_desktop_install EXIT HUP INT TERM

	echo "Downloading FlowReco Desktop Debian package..."
	...
	if command -v apt-get >/dev/null 2>&1; then
		...
		echo "Could not find apt-get or dpkg. FlowReco for Linux currently ships as a Debian package." >&2
		exit 1
	fi
	...
	if ! find_linux_cli_target; then
		echo "This FlowReco Desktop package does not include the CLI." >&2
		exit 1
	fi
}
...
install_cap_desktop() {
	case "$OS_NAME" in
		...
		Linux)
			...
			;;
		*)
			echo "FlowReco Desktop auto-install is only supported on macOS and Linux. Install FlowReco from https://cap.so/download, then run this script again." >&2
			exit 1
			;;
	esac
}
...
resolve_cli_target() {
	case "$OS_NAME" in
		Darwin)
			if [ -z "$APP_PATH" ]; then
				...
				echo "FlowReco Desktop was not found at $APP_PATH." >&2
				exit 1
			fi
			...
			if [ ! -x "$CLI_TARGET" ]; then
				echo "This FlowReco Desktop install does not include the CLI. Reinstalling FlowReco Desktop..."
				install_cap_desktop
				CLI_TARGET="$APP_PATH/Contents/MacOS/cap-cli"
			fi
			;;
	...
	if [ ! -x "$CLI_TARGET" ]; then
		echo "This FlowReco Desktop install does not include the CLI." >&2
		exit 1
	fi
}
...
if [ -e "$SHIM_PATH" ] || [ -L "$SHIM_PATH" ]; then
	if [ ! -L "$SHIM_PATH" ]; then
		echo "$SHIM_PATH already exists and is not managed by FlowReco. Remove it or set CAP_CLI_INSTALL_DIR, then run this script again." >&2
		exit 1
	fi
	...
	case "$EXISTING_TARGET" in
		"$CLI_TARGET"|*/FlowReco.app/Contents/MacOS/cap-cli|*/cap-cli|*/cap-cli-*linux*)
			;;
		*)
			echo "$SHIM_PATH already exists and is not managed by FlowReco. Remove it or set CAP_CLI_INSTALL_DIR, then run this script again." >&2
			exit 1
...
case ":$PATH:" in
	...
	*)
		if [ -n "${capNoModifyPathParameter}" ]; then
			...
			elif printf '\n# Added by the FlowReco CLI installer\n%s\n' "$EXPORT_LINE" >> "$PROFILE" 2>/dev/null; then
