...
const getCapDetailErrorMessage = (error: unknown) =>
	error instanceof Error ? error.message : "Unable to load this recording";
...
export default function CapDetailScreen() {
	...
	const actionInProgressHint = "Current recording action is in progress";
	...
	const commentHint = isPostingComment
		...
		: isActionInProgress
			? actionInProgressHint
			: "Add a comment to this recording";
	...
	const deleteCap = () => {
			...
			ActionSheetIOS.showActionSheetWithOptions(
				{
					...
					message: "This recording will be removed from your library.",
		...
		Alert.alert(
			"Delete Recording",
			"This recording will be removed from your library.",
			[
				{ text: "Cancel", style: "cancel" },
				{
					text: "Delete",
		...
		>
			<Stack.Screen
				options={{
					...
					title: detail?.cap.title ?? "FlowReco",
				}}
			/>
			<Screen loading={loading} scroll safeEdges={["left", "right"]}>
				{loadError ? (
					<View
						accessibilityLabel={`Recording detail error: ${loadError}`}
					...
					>
						...
						<ActionButton
							label="Try again"
							accessibilityHint="Reloads this recording"
							onPress={() => {
								void load();
							}}
					...
					<>
						...
						<View style={styles.actions}>
							<ActionButton
								...
								accessibilityHint="Copies this recording link"
								variant="secondary"
								onPress={copyLink}
								...
								<View key={item.id} style={styles.comment}>
									...
									<View style={styles.commentBody}>
										<Text numberOfLines={1} style={styles.commentAuthor}>
											{item.author.name ?? "FlowReco user"}
										</Text>
