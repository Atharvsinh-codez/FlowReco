...
const getCapsErrorMessage = (error: unknown) => {
	if (error instanceof MobileApiError) {
		...
		return "FlowReco could not load your library. Try again.";
	}
	return error instanceof Error
		? error.message
		: "FlowReco could not load your library. Try again.";
};
...
const showPhotosSettingsAlert = () => {
	if (Platform.OS === "ios") {
		ActionSheetIOS.showActionSheetWithOptions(
			{
				cancelButtonIndex: 1,
				message: "Allow FlowReco to save videos to Photos from Settings.",
	...
	Alert.alert(
		...
		"Allow FlowReco to save videos to Photos from Settings.",
		[
...
export default function CapsScreen() {
				...
				ActionSheetIOS.showActionSheetWithOptions(
					{
						...
						options: ["Delete Recording", "Cancel"],
			...
			try {
				...
				Alert.alert(
					...
					error instanceof Error
						? error.message
						: "Unable to update sharing for this recording.",
				);
			} finally {
	...
	const folderCreationHint = creatingFolder
		...
		: "Creates a folder for organizing recordings";
	...
	const saveDisabledHint =
		savingId !== null
			? "Save is in progress"
			: "Current recording action is in progress";
	...
	if (auth.status === "loading") {
		return <Screen title="Recordings" loading />;
	}
	...
	return (
		<Screen title="Recordings" loading={loading && !result}>
			{auth.bootstrap ? (
			...
			{folder ? (
				...
				>
					<Text style={styles.folderCrumbText}>Recordings</Text>
					<SymbolView
						name="chevron.right"
						size={14}
			...
			{loadError ? (
				...
				>
					...
					<View style={styles.errorCopy}>
						<Text style={styles.errorTitle}>Unable to load recordings</Text>
						<Text style={styles.errorText}>{loadError}</Text>
					</View>
					<ActionButton
						label="Try again"
						accessibilityHint="Reloads your FlowReco library"
						onPress={load}
						size="sm"
					...
					ListEmptyComponent={
						<View style={styles.emptyState}>
							...
							<Text style={styles.emptyTitle}>
								Hey{userName ? ` ${userName}` : ""}! Import your first recording
							</Text>
							<Text style={styles.emptyText}>
								Bring videos into FlowReco and share them instantly.
							</Text>
