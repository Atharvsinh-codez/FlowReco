...
describe("CapCard", () => {
	it("uses a branded thumbnail placeholder when a FlowReco has no thumbnail", async () => {
	...
	it("shows copy, share, and more actions together", async () => {
		...
		expect(hasProp(tree, "accessibilityHint", "Opens FlowReco actions")).toBe(
			true,
		);
	});

	it("uses the FlowReco web neutral button surface for card actions", async () => {
		const renderer = await renderComponent(
			React.createElement(CapCard, {
				cap,
				onPress: vi.fn(),
