...
describe("createMobileApiClient", () => {
	...
	it("decodes public auth provider config", async () => {
		...
		try {
			...
			expect(String(calls[0])).toBe(
				"http://localhost:3000/api/mobile/session/config",
			);
		} finally {
			globalThis.fetch = originalFetch;
		}
	...
	it("builds Google session request URLs", () => {
		expect(
			...
			"http://localhost:3000/api/mobile/session/request?redirectUri=cap%3A%2F%2Fauth&provider=google",
		);
	});
	...
	it("builds WorkOS session request URLs", () => {
		expect(
			...
			"http://localhost:3000/api/mobile/session/request?redirectUri=cap%3A%2F%2Fauth&provider=workos&organizationId=org_123",
		);
	});

	it("updates FlowReco sharing with the authenticated PATCH endpoint", async () => {
		...
		globalThis.fetch = (async (
			...
			return new Response(
				JSON.stringify({
					id: "video_123",
					shareUrl: "http://localhost:3000/s/video_123",
					title: "Launch review",
		...
		try {
			...
			expect(String(calls[0]?.input)).toBe(
				"http://localhost:3000/api/mobile/caps/video_123/sharing",
			);
	...
	it("creates folders with the authenticated POST endpoint", async () => {
		...
		try {
			...
			expect(String(calls[0]?.input)).toBe(
				"http://localhost:3000/api/mobile/folders",
			);
	...
	it("updates FlowReco titles with the authenticated PATCH endpoint", async () => {
		const calls: Array<{ input: RequestInfo | URL; init?: RequestInit }> = [];
		const originalFetch = globalThis.fetch;
		globalThis.fetch = (async (
			input: RequestInfo | URL,
		...
		try {
			...
			expect(String(calls[0]?.input)).toBe(
				"http://localhost:3000/api/mobile/caps/video_123/title",
			);
			expect(calls[0]?.init?.method).toBe("PATCH");
			expect(typeof body).toBe("string");
	...
	it("updates FlowReco passwords with the authenticated PATCH endpoint", async () => {
		const calls: Array<{ input: RequestInfo | URL; init?: RequestInit }> = [];
		const originalFetch = globalThis.fetch;
		globalThis.fetch = (async (
			input: RequestInfo | URL,
		...
		try {
			...
			expect(String(calls[0]?.input)).toBe(
				"http://localhost:3000/api/mobile/caps/video_123/password",
			);
			expect(calls[0]?.init?.method).toBe("PATCH");
			expect(typeof body).toBe("string");
