...
describe("FlowReco detail screen", () => {
	beforeEach(() => {
		vi.clearAllMocks();
		authState.value = createAuth();
	});

	it("announces FlowReco detail load errors with a retry action", async () => {
		...
		expect(text).toContain("Unable to load FlowReco");
		...
		expect(
			hasProp(
				tree,
				"accessibilityLabel",
				"FlowReco detail error: Network unavailable",
			),
		).toBe(true);
		...
		if (!retryButton)
			throw new Error("FlowReco detail retry action was not rendered");
		expect(retryButton.props.accessibilityHint).toBe("Reloads this FlowReco");
	...
	it("shows web-matching sharing, analytics, and action labels", async () => {
		...
		expect(
			hasProp(tree, "accessibilityHint", "Copies this FlowReco link"),
		).toBe(true);
		expect(
	...
	it("uses native affordances for the header menu and comment composer", async () => {
		...
		expect(headerAction.props.accessibilityHint).toBe(
			"Opens FlowReco settings",
		);
	...
	it("opens analytics in the native browser sheet", async () => {
		...
		expect(openBrowserAsync).toHaveBeenCalledWith(
			"http://localhost:3000/dashboard/analytics?capId=video_123",
		);
	});
	...
	it("shows a save-specific busy state without blocking sharing as saving", async () => {
		...
		expect(sheet?.props.visibilityDisabledHint).toBe(
			"Current FlowReco action is in progress",
		);
	...
	it("marks the settings sheet sharing row as updating during visibility changes", async () => {
		...
		if (!sheet) throw new Error("FlowReco settings sheet was not rendered");
