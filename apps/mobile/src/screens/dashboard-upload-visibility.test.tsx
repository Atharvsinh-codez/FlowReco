...
describe("upload and dashboard visibility", () => {
	...
	it("opens the native iOS upload source sheet", async () => {
		...
		expect(openBrowserAsync).toHaveBeenCalledWith(
			"http://localhost:3000/dashboard/import/loom",
		);
	});
	...
	it("shows the active Photos source as loading while the photo picker is opening", async () => {
		...
		expect(showActionSheetWithOptions).toHaveBeenCalledWith(
			expect.objectContaining({
				cancelButtonIndex: 1,
				message: "Allow FlowReco to read videos from Photos before uploading.",
		...
		expect(getTextNodes(renderer.toJSON())).toContain(
			"Allow FlowReco to read videos from Photos before uploading.",
		);
		...
		expect(failedUploadSource.props.accessibilityValue).toEqual({
			text: "Allow FlowReco to read videos from Photos before uploading.",
		});
	...
	it("shows dashboard import actions", async () => {
		...
		expect(text.join("")).toContain("Hey Richie! Import your first FlowReco");
		expect(hasProp(tree, "accessibilityLabel", "FlowReco logo")).toBe(true);
		expect(
			hasProps(tree, {
	...
	it("marks the dashboard card sharing action busy while visibility is updating", async () => {
		...
		if (!capCard) throw new Error("FlowReco card was not rendered");
		...
		if (!busyCard) throw new Error("Busy FlowReco card was not rendered");
