...
describe("SignInPanel", () => {
	...
	it("renders the FlowReco web login surface", async () => {
		const tree = await renderTree(React.createElement(SignInPanel));
		const text = getTextNodes(tree);

		expect(text).toContain("Sign in to FlowReco");
		...
		expect(hasProp(tree, "selectionColor", "#ff6243")).toBe(true);
		expect(
			hasProp(
				tree,
				"accessibilityHint",
	...
	it("locks the email field while requesting a verification code", async () => {
		...
		await act(async () => {
			emailInput.props.onChangeText("richie@flowreco.test");
		});
		await act(async () => {
		...
		await act(async () => {
			emailInput.props.onChangeText("changed@flowreco.test");
		});

		const [unchangedEmailInput] = renderer.root.findAllByProps({
			placeholder: "tim@apple.com",
		});
		expect(unchangedEmailInput?.props.value).toBe("richie@flowreco.test");
	...
	it("shows the right error when an email is not allowed", async () => {
		...
		expect(getTextNodes(renderer.toJSON())).toContain(
			"This email cannot be used to sign in to FlowReco.",
		);
		...
		expect(emailButtonAfterError?.props.accessibilityValue).toEqual({
			text: "This email cannot be used to sign in to FlowReco.",
		});
		expect(errorAlert?.props.accessibilityLabel).toBe(
			"Sign-in error: This email cannot be used to sign in to FlowReco.",
		);
	...
	it("switches to a web-like verification code step after requesting email", async () => {
		...
		expect(authFns.requestEmailCode).toHaveBeenCalledWith(
			"richie@flowreco.test",
		);
		expect(text).toContain("Back");
		expect(text).toContain("Enter verification code");
		expect(text).toContain("We sent a 6-digit code to richie@flowreco.test");
	...
	it("allows a corrected email to request a code without waiting for the previous cooldown", async () => {
		...
		await act(async () => {
			emailInput.props.onChangeText("wrong@flowreco.test");
		});
		await act(async () => {
			await emailButton.props.onPress();
		});
		...
		await act(async () => {
			correctedEmailInput.props.onChangeText("right@flowreco.test");
		});
		...
		expect(authFns.requestEmailCode).toHaveBeenNthCalledWith(
			1,
			"wrong@flowreco.test",
		);
		expect(authFns.requestEmailCode).toHaveBeenNthCalledWith(
