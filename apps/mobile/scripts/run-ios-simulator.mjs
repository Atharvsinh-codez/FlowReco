...
const needsDevPrebuild = () => {
	if (
		existsSync(join(process.cwd(), "ios", "FlowRecoBroadcastExtension")) ||
		existsSync(join(process.cwd(), "ios", "CapBroadcastExtension"))
	) {
		return true;
	}
	const entitlementsPath = [
		join(process.cwd(), "ios", "FlowReco", "FlowReco.entitlements"),
		join(process.cwd(), "ios", "Cap", "Cap.entitlements"),
	].find((path) => existsSync(path));
	if (!entitlementsPath) return true;
...
if (
	process.env.FLOWRECO_MOBILE_DRY_RUN === "1" ||
	process.env.CAP_MOBILE_DRY_RUN === "1"
) {
