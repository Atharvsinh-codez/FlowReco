//! Runs FlowReco automation rules from the CLI, sharing the exact rule model and engine the desktop app
//! uses (`cap_automation`). Rules are authored in FlowReco Desktop and persisted to its tauri-plugin-store
...
/// `(total_rules, enabled_rules)` configured in FlowReco Desktop, for `cap doctor`.
...
impl AutomationHost for CliAutomationHost {
    ...
    async fn apply_preset(&self, ctx: &TriggerContext, name: &str) -> Result<(), String> {
        ...
        let store = load_desktop_store_value().ok_or("FlowReco Desktop store not found")?;
        let presets = store
            .get("presets")
            .and_then(|p| p.get("presets"))
            .and_then(Value::as_array)
            .ok_or("No presets found in FlowReco Desktop store")?;
...
/// `cap automations list` — print the automation rules shared with FlowReco Desktop.
pub fn list(format: crate::OutputFormat) -> Result<(), String> {
    ...
    match format {
        ...
        crate::OutputFormat::Text => {
            if store.rules.is_empty() {
                println!(
                    "No automations configured. Add them in FlowReco Desktop under Settings > Automations."
                );
                return Ok(());
            }
