...
const ANSI_CORAL: &str = "\x1b[38;2;255;98;67m";
const ANSI_MUTED: &str = "\x1b[38;2;118;128;145m";

const WELCOME_LINES: &[&[(&str, &str)]] = &[
    &[(ANSI_CORAL, "  ●  "), (ANSI_BOLD, "FLOWRECO")],
    &[],
    &[(
        ANSI_BOLD,
    ...
    &[],
    &[(ANSI_CORAL, "  cap record start --screen <id> --detach")],
    &[(
        ANSI_MUTED,
...
AUTH
  `cap upload` authenticates automatically by reusing the login FlowReco Desktop already stored — no
  key to copy when you are signed in there. Check with `cap auth status --json`. For headless/CI,
	set FLOWRECO_API_KEY to a FlowReco auth key (Settings) to override.

ENVIRONMENT
	FLOWRECO_API_KEY     Overrides auth for `cap upload`; optional when signed into FlowReco Desktop.
	FLOWRECO_SERVER_URL FlowReco server base URL; defaults to FlowReco Desktop, then localhost.
	CAP_API_KEY and CAP_SERVER_URL remain compatibility aliases.
  CAP_NO_MODIFY_PATH  Set to skip editing shell profiles during `cap desktop install-cli`.
  CAP_DESKTOP_FORCE_INSTALL
                      Force the web installer scripts to replace FlowReco Desktop before linking the CLI.
...
TYPICAL AGENT WORKFLOW
  ...
  cap export <path.cap> --output out.mp4 --json
	cap upload out.mp4 --json                   # get a shareable link (needs configured auth)";

#[derive(Parser)]
#[command(
    name = "cap",
    version,
    about = "FlowReco screen recording from the command line",
    long_about = "FlowReco screen recording from the command line.\n\nDesigned to be driven by automation and AI agents: add --json to any command for \
...
enum Commands {
    ...
    /// Show FlowReco's signed-release update status
    Update(FormatArgs),
    /// Show how `cap upload` will authenticate (env key or FlowReco Desktop login)
    Auth(AuthArgs),
    /// List available capture targets and devices
    Targets(TargetsArgs),
    /// Report CLI environment and capture-readiness diagnostics
    Doctor(FormatArgs),
    /// Run end-to-end diagnostics that verify FlowReco works on this machine
    ...
    Guide(FormatArgs),
    /// List automation rules shared with FlowReco Desktop
...
enum AutomationsCommands {
    /// List the automation rules configured in FlowReco Desktop
    List(FormatArgs),
}
