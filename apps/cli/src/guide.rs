...
fn build() -> Guide {
    Guide {
        ...
        description: "FlowReco screen recording, driven from the command line. Add --json to any command \
        ...
        env: vec![
            EnvVar {
                name: "FLOWRECO_API_KEY",
                required: false,
                used_by: "upload",
                description: "Overrides upload auth. Optional when signed into FlowReco Desktop.",
            },
            EnvVar {
                name: "FLOWRECO_SERVER_URL",
                required: false,
                used_by: "upload",
                description: "FlowReco server base URL. Defaults to Desktop settings, then http://localhost:3000.",
            },
            EnvVar {
                name: "CAP_API_KEY",
                required: false,
                used_by: "upload",
                description: "Compatibility alias for FLOWRECO_API_KEY.",
            },
            EnvVar {
                name: "CAP_SERVER_URL",
                required: false,
                used_by: "upload",
                description: "Compatibility alias for FLOWRECO_SERVER_URL.",
            },
            EnvVar {
            ...
            EnvVar {
                ...
                description: "Force the installer scripts to replace FlowReco Desktop before linking the CLI.",
            },
        ],
        exit_codes: vec![
            ExitCode {
        ...
        commands: vec![
            ...
            cmd(
                "upload",
                "Upload a .cap project or video file; returns a shareable link. Authenticates via FlowReco Desktop's login or CAP_API_KEY.",
                OutputMode::SingleJson,
                &[],
            ),
            cmd(
                "update",
                "Download and install the latest FlowReco Desktop bundle, then repair the `cap` shim.",
                OutputMode::SingleJson,
                &[],
            ),
            cmd(
            ...
            cmd(
                "automations list",
                "List the automation rules configured in FlowReco Desktop (Settings > Automations) that the CLI honors after screenshot/record/upload.",
                OutputMode::SingleJson,
                &[],
            ),
            cmd(
        ...
        notes: vec![
            ...
            "Automations authored in FlowReco Desktop run automatically after `cap screenshot`, `cap record` \
