//! `cap selftest` — diagnostics that verify FlowReco works correctly on this
...
async fn run_av_sync(args: AvSyncArgs, json: bool) -> Result<(), String> {
    ...
    progress(json, "FlowReco A/V sync self-test");
    progress(
        json,
        &format!(
        ...
        .map_err(|e| {
            format!(
                "screen recording permission unavailable: {e}. \
                 Grant FlowReco screen recording access in System Settings and retry."
            )
        })
