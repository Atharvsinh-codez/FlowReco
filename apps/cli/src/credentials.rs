...
//!
//! Priority: explicit FlowReco environment variables (with `CAP_*` aliases for compatibility), then the login the
//! desktop app already stored. The CLI is the same product as FlowReco Desktop, so if the user is signed
...
pub enum CredentialSource {
    /// FLOWRECO_API_KEY or legacy CAP_API_KEY environment variable.
    Env,
    /// The login stored by FlowReco Desktop.
    Desktop,
    /// No credential found.
    None,
}
...
fn compatible_env(primary: &str, legacy: &str) -> Option<String> {
    env_var(primary).or_else(|| env_var(legacy))
}
...
pub fn resolve() -> Result<Credentials, String> {
    ...
    let server = normalize_server(
        compatible_env("FLOWRECO_SERVER_URL", "CAP_SERVER_URL")
            .or_else(|| store.as_ref().and_then(store_server))
            .unwrap_or_else(|| DEFAULT_SERVER.to_string()),
    );

    if let Some(api_key) = compatible_env("FLOWRECO_API_KEY", "CAP_API_KEY") {
    ...
    Err(
        "Not signed in. Sign in to FlowReco Desktop (the CLI reuses its login), or set \
         FLOWRECO_API_KEY to an auth key from Settings. CAP_API_KEY remains a compatibility alias."
            .to_string(),
    )
}
        ...
        OutputFormat::Text => {
            if status.authenticated {
                let source = match status.source {
                    CredentialSource::Env => "FlowReco API key environment variable",
                    CredentialSource::Desktop => "FlowReco Desktop login",
