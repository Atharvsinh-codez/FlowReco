...
const createMockCapServer = async (
	...
	const server = createServer(async (request, response) => {
		...
		if (request.method === "GET" && url.pathname === "/capture.html") {
			...
			response.end(
				`<!doctype html><html><head><title>FlowReco Close UI Target</title></head><body style="background:#321"><h1>Close UI page</h1></body></html>`,
			);
			return;
		}
		if (
...
const launchExtensionContext = async () => {
	...
	const context = await chromium.launchPersistentContext(userDataDir, {
		...
		args: [
			...
			"--auto-select-tab-capture-source-by-title=FlowReco Close UI Target",
			"--auto-select-desktop-capture-source=FlowReco Close UI Target",
...
test("clicking X in the panel closes the panel and the camera preview", async () => {
	...
	try {
		...
		await panelFrame
			.locator('button[aria-label="Close FlowReco and hide all recorder UI"]')
			.click();
