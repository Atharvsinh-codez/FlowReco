...
const maybeExposeFlowRecoWebBridge = () => {
	if (isLocalFlowRecoOrigin()) {
		exposeFlowRecoWebBridge();
		return;
	}
	...
	chrome.storage.local.get([SETTINGS_KEY], (items) => {
		...
		const apiBaseUrl = (saved as { apiBaseUrl?: unknown }).apiBaseUrl;
		if (typeof apiBaseUrl !== "string") return;
		try {
			if (new URL(apiBaseUrl).origin === window.location.origin) {
				exposeFlowRecoWebBridge();
			}
		} catch {
			// Invalid saved URLs are handled in the extension Options page.
		}
	});
};
