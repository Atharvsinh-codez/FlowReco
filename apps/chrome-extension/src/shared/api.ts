...
const checkAuthStartRoute = async (
	...
	try {
		...
		throw new Error(
			`Could not reach FlowReco extension auth at ${url.origin}. Start the local web server with pnpm dev:extension and make sure the FlowReco Server URL in extension Options is ${settings.apiBaseUrl}. Redirect URI: ${redirectUri}. ${message}`,
		);
	}
};
