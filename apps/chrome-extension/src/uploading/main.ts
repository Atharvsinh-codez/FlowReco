...
const RETRY_SILENT_FAILURE_MESSAGE =
	"The retry did not report any progress. It may still be running - check your recordings before retrying again.";
...
const syncTabTitle = () => {
	...
	lastTitleKey = key;
	if (mode === "completed") {
		document.title = "Ready - FlowReco";
		return;
	}
	if (mode === "error") {
		document.title = "Upload needs attention - FlowReco";
		return;
	}
	if (mode === "waiting") {
		document.title = "Uploading - FlowReco";
		return;
	}
	document.title = `Uploading ${percent}% - FlowReco`;
};
...
const renderActiveStatus = (status: ActiveStatus) => {
	...
	if (status.phase === "recording") {
		...
		applyStageState(
			"recording",
			"Still recording",
			"Your recording streams to the cloud while you record.",
		);
		return;
	}
...
const pollStatus = async () => {
	...
	if (consecutivePollFailures >= 10 && mode === "waiting") {
		detailElement.textContent = "Still trying to reach the FlowReco extension…";
	}
};
