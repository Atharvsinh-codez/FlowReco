		...
		chrome.offscreen.createDocument(
			{
				...
				justification:
					"Record and upload FlowReco videos from an extension page.",
			},
			() => {
...
const updateActionForStatus = (nextStatus: RecordingStatus) => {
	...
	return Promise.all([
		...
		setActionTitle(
			isCapturing
				? "Stop FlowReco recording"
				: "Record your screen with FlowReco",
		),
	]).then(() => undefined);
};
...
const requireSignedInState = async () => {
	...
	if (!state.auth || !state.bootstrap) {
		throw new Error("Sign in to FlowReco first");
	}
	return state as {
		settings: ExtensionSettings;
		auth: ExtensionAuth;
		...
		if (auth) {
			try {
				...
				if (status === null || status >= 500) {
					throw new Error(
						"Could not reach FlowReco to revoke this sign-in. Check your connection and try again.",
					);
				}
			}
		}
