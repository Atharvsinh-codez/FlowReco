...
export const SignInView = ({
	...
	return (
		<div className="cap-signin">
			<FlowRecoBrand className="cap-signin-brand cap-fade-up" />
			<svg
			...
			<p className="cap-signin-lede cap-fade-up cap-fade-up-3">
				{authPending
					? "Complete sign-in in the FlowReco window. This panel updates automatically."
			...
			{authPending ? (
				<p className="cap-signin-wait cap-fade-up cap-fade-up-4">
					...
					Waiting for the FlowReco sign-in window…
				</p>
			) : null}
			<button
				type="button"
				...
				onClick={onSignIn}
			>
				{authPending ? "Open the sign-in window again" : "Sign in to FlowReco"}
			</button>
			<p
				className={
					authPending
